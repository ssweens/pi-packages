...
- v1.2.2 catalog checks run against `models/index.ts`: 52 models load, no duplicate ids, the retired `gemini-2.0-*` ids no longer resolve, and `gemini-3.8-flash`, `glm-5.2`, and `grok-4.6` resolve with their expected API ids, context windows, output caps, and pricing.
...
- `bun build index.ts --outdir /tmp/pi-vertex-build-maas --target node` completes successfully.

## Gaps / Next Steps
- **No live request has been made against `gemini-3.8-flash`, `gemini-3.7-flash`, `gemini-3.6-flash`, `gemini-3.5-flash-lite`, `claude-opus-5`, `claude-sonnet-5`, `glm-5.2`, or `grok-4.6`.** Catalog metadata and thinking routing are verified statically; end-to-end streaming against these API ids is unverified.
