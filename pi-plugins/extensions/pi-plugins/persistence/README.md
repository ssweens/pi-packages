...
State persistence and filesystem location resolution. `state.json` schema + load/save with legacy-record migration; `ScopedLocations` typed bundle (the `<scopeRoot>/pi-plugins/`, `<scopeRoot>/agents/`, `<scopeRoot>/mcp.json` triple). Phase 2 lands both.
