# Product Requirements Document: pi-plugins
...
- [Product Requirements Document: pi-plugins](#product-requirements-document-pi-plugins)
...
`pi-plugins` is a Pi extension that gives Pi users access to Claude plugin marketplaces using a command surface intentionally aligned with Claude Code's upstream `/plugin`. The extension translates Claude plugin artefacts (skills, commands, agents, MCP servers) into the equivalent Pi-native artefacts (Pi skills, Pi prompt templates, pi-subagents agents, pi-mcp-adapter MCP entries) and manages the lifecycle of those translations (install, update, uninstall, remove, list).
...
- **Generated name** -- the deterministic name produced for a Pi-side artefact: `<plugin>-<skill>` for skills, `<plugin>:<command>` for commands, `pi-plugins-<plugin>-<agent>` for agents.
...
1. **`<scope>/pi-plugins/state.json`** -- single source of truth for which marketplaces are configured, source kinds, plugin install records (skills, prompts, agents, mcpServers), versions, autoupdate flags, etc.
2. **`<scope>/pi-plugins/resources/{skills,prompts}/`** -- generated Pi skill directories and prompt template files, exposed back to Pi via `resources_discover`.
3. **Out-of-extension bridge files** for soft deps:
   - `<scope>/agents/pi-plugins-*.md` (with a separate `<extension>/agents-index.json`).
   - `<scope>/mcp.json` (entries marked with `_piPlugins: { plugin, marketplace }`).
...
| **MA-5**  | For GitHub sources, the system MUST `git clone` into `<staging>/<uuid>/`, read the manifest, and only after a successful read rename into `<scope>/pi-plugins/sources/<marketplace-name>/`.                                                |
...
| **PU-7** | Foreign content at an agent target file (basename does not start with `pi-plugins-` or file is missing the generated marker) MUST cause that entry to be retained in the index with `failed[]` rather than silently kept. The uninstall MUST fail loudly. |
...
| ID       | Requirement                                                                                                                                                                                                                                                                                 |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SK-1** | Skills MUST be staged as Pi skills under `<scope>/pi-plugins/resources/skills/<plugin>-<skill>/SKILL.md` (with the entire skill directory copied recursively).                                                                                                                   |
...
| ID       | Requirement                                                                                                                  |
| -------- | ---------------------------------------------------------------------------------------------------------------------------- |
| **CM-1** | Commands MUST be staged as `<scope>/pi-plugins/resources/prompts/<plugin>:<command>.md`.                          |
...
| ID        | Requirement                                                                                                                                                                                                                                                                                                                               |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AG-1**  | Agent files MUST be staged at `<scope>/agents/pi-plugins-<plugin>-<agent>.md` (deliberately outside the extension's `resources/` because pi-subagents reads from a fixed convention path).                                                                                                                                     |
...
| **AG-5**  | Generated agent files MUST start with `pi-plugins-` (basename) AND contain the literal marker string `generated by pi-plugins` inside an HTML-comment provenance block placed immediately after the closing `---` of the frontmatter. Removal MUST refuse to touch any file failing either check (foreign content). |
...
| **MC-5** | Each staged entry MUST carry a `_piPlugins: { plugin, marketplace }` marker so uninstall/update drops only its own entries.                                                                                                                                                                                                                                                                                                                           |
...
| **SC-2** | All extension data lives at `<scopeRoot>/pi-plugins/`. Bridge files live at `<scopeRoot>/agents/` and `<scopeRoot>/mcp.json` (deliberately outside the extension's resources/).                                                                                                                  |
...
| ID       | Requirement                                                                                                                                                                                                                                                                                                                                         |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RN-1** | Generated names MUST be deterministic functions of `(plugin, source-name)`. Skill: `<plugin>-<skill>` (prefix elided). Command: `<plugin>:<command>` (prefix elided). Agent: `pi-plugins-<plugin>-<agent>` (with `<plugin>-` prefix on source elided).                                                                                   |
...
```mermaid
flowchart LR
  ext[pi-plugins] --> probe[hasLoadedPiSubagents / hasLoadedPiMcpAdapter]
...
| **NFR-10** | The system MUST refuse to write outside `<scopeRoot>/pi-plugins/`, `<scopeRoot>/agents/`, or `<scopeRoot>/mcp.json` for any reason.                                                                                                                                                                                        |
...
| Agent      | `pi-plugins-<plugin>-<agent>` | drop leading `<plugin>-` from source       |
| MCP server | server name verbatim from declaration    | none (`_piPlugins` marker added) |
