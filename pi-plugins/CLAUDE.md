...
This GSD project plans a successor architecture for the V1 implementation already in this repository. The PRD at `docs/prd/pi-plugins-prd.md` (1068 lines, v1.0) is derived from the V1 source and is the authoritative specification of what the successor must deliver.
...
| **`fs.cp({ recursive: true })` for committing staging trees across filesystems** | If staging dir and destination are on different filesystems, `rename` will EXDEV-fail. NFR-1's atomic guarantee requires same-FS.                                             | Place staging dirs on the same scope root (V1 already does -- `agents-staging/` lives under `<scope>/pi-plugins/`); for any cross-FS ops, fall back to copy-then-fsync-then-unlink, never claim atomicity |
