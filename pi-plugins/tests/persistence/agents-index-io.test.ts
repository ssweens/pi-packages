...
import {
  loadAgentsIndex,
  saveAgentsIndex,
} from "../../extensions/pi-plugins/persistence/agents-index-io.ts";
...
async function tmpScope(): Promise<TmpScope> {
  ...
  // locationsFor("project", cwd) -> extensionRoot = <cwd>/.pi/pi-plugins
...
test("AG-2 loadAgentsIndex round-trips a single-row index from the single-row.json fixture", async () => {
  ...
  try {
    ...
    assert.equal(got.agents[0]?.generatedName, "pi-plugins-acme-bot");
