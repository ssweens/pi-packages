// Plan 06-04 Task 2: pi_plugins_list + pi_plugins_plugin_list
// LLM-tool tests.
//
...
import { pathSource } from "../../../extensions/pi-plugins/domain/source.ts";
import {
...
import { saveState } from "../../../extensions/pi-plugins/persistence/state-io.ts";
...
test("D-02 :: registerListMarketplacesTool registers tool name pi_plugins_list with empty params schema", () => {
  const { pi, registered } = makeMockPi();
  registerListMarketplacesTool(pi);
  assert.equal(registered.size, 1);
  const tool = registered.get("pi_plugins_list");
...
test('pi_plugins_list :: empty state returns content text "No marketplaces configured." + details.marketplaces == []', async () => {
...
test("pi_plugins_list :: populated state returns one line per marketplace formatted [<scope>] <name> -- <N> plugin(s) -- <source.logical>", async () => {
  await withHermeticHome(async ({ cwd }) => {
...
// ─── pi_plugins_plugin_list ──────────────────────────────────────

test("D-02 :: registerListPluginsTool registers tool name pi_plugins_plugin_list with extended params", () => {
  const { pi, registered } = makeMockPi();
  registerListPluginsTool(pi);
  assert.equal(registered.size, 1);
  const tool = registered.get("pi_plugins_plugin_list");
  assert.notEqual(tool, undefined);
  assert.notEqual(tool!.parameters, undefined);
});

test("pi_plugins_plugin_list :: marketplace set, marketplace exists -> plugins from that marketplace", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: marketplace set, marketplace not found -> error text + details.plugins == []", async () => {
  await withHermeticHome(async ({ cwd }) => {
    const { pi, registered } = makeMockPi();
    registerListPluginsTool(pi);
    const tool = registered.get("pi_plugins_plugin_list")!;
...
test("pi_plugins_plugin_list :: marketplace omitted -> enumerate across all marketplaces", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: installed: true filter -> only installed bucket", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: available: true filter -> only available bucket", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: unavailable: true filter -> only unavailable bucket", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: available: true + unavailable: true -> union of both (PL-1)", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test("pi_plugins_plugin_list :: no filters -> all three buckets (PL-1 default)", async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "project",
...
test('pi_plugins_plugin_list :: scope: "user" filters to user scope only', async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "user",
...
test('pi_plugins_plugin_list :: scope: "project" filters to project scope only', async () => {
  await withHermeticHome(async ({ cwd }) => {
    await seedMarketplace({
      cwd,
      scope: "user",
