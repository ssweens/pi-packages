...
import type { ImportClaudeSettingsOptions } from "../../extensions/pi-plugins/orchestrators/import/execute.ts";
import type {
  PluginUpdateFn,
  PluginUpdateOutcome,
} from "../../extensions/pi-plugins/orchestrators/types.ts";
...
test("D-04 :: registerClaudeMarketplaceTools registers pi_plugins_list", () => {
  const { pi, tools } = makeMockPi();
  registerClaudeMarketplaceTools(pi);

  assert.ok(tools.has("pi_plugins_list"), "pi_plugins_list registered");
});

test("D-04 :: registerClaudeMarketplaceTools registers pi_plugins_plugin_list", () => {
  const { pi, tools } = makeMockPi();
  registerClaudeMarketplaceTools(pi);

  assert.ok(tools.has("pi_plugins_plugin_list"), "pi_plugins_plugin_list registered");
});
