 ...
 * migration.
 */
test("AG-5 GENERATED_AGENT_MARKER is byte-for-byte 'generated by pi-plugins'", () => {
  assert.equal(GENERATED_AGENT_MARKER, "generated by pi-plugins");
});

test("AG-5 GENERATED_AGENT_PREFIX is byte-for-byte 'pi-plugins-'", () => {
 ...
 */
test("D-08 state-lock-held prefix is byte-for-byte 'Another pi-plugins operation is in progress for'", () => {
  assert.equal(markers.STATE_LOCK_HELD_PREFIX, "Another pi-plugins operation is in progress for");
});
