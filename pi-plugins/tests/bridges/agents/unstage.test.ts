...
} from "../../../extensions/pi-plugins/bridges/agents/marker.ts";
import { unstagePluginAgents } from "../../../extensions/pi-plugins/bridges/agents/unstage.ts";
...
function makeOwnedFileContent(name: string): string {
  // Minimum viable owned file: basename starts with pi-plugins- (we
...
test("unstagePluginAgents removes owned files and updates index (happy path)", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    await writeFile(a, makeOwnedFileContent("pi-plugins-acme-bot"));
    await writeFile(b, makeOwnedFileContent("pi-plugins-acme-helper"));
    ...
    const seed: AgentsIndex = {
      ...
      agents: [
        ...
        },
        {
          ...
          marketplace: "mp1",
          sourceAgent: "helper",
          generatedName: "pi-plugins-acme-helper",
...
test("AG-3 unstage of plugin X in mp1 leaves plugin X in mp2 rows untouched", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    await writeFile(mp1Target, makeOwnedFileContent("pi-plugins-acme-bot"));
    await writeFile(mp2Target, makeOwnedFileContent("pi-plugins-acme-mp2only"));
...
test("AG-5 unstage SOFT-FAILS on foreign content (basename mismatch)", async () => {
  await withTmpScope(async ({ locations }) => {
    await mkdir(locations.agentsDir, { recursive: true });
    // Index points at a foreign path (no pi-plugins- prefix).
    ...
    const after = await loadAgentsIndex(locations);
    assert.equal(after.agents.length, 1);
    assert.equal(after.agents[0]?.generatedName, "pi-plugins-acme-ghost");
  });
});
...
test("unstagePluginAgents tolerates ENOENT on target file (treats as removed)", async () => {
  await withTmpScope(async ({ locations }) => {
      ...
      agents: [
        {
          ...
          marketplace: "mp1",
          sourceAgent: "phantom",
          generatedName: "pi-plugins-acme-phantom",
    ...
    assert.deepEqual([...result.removedNames], ["pi-plugins-acme-phantom"]);
    assert.equal(result.failed.length, 0);
      ...
      agents: [
        {
          ...
          marketplace: "mp2",
          sourceAgent: "x",
          generatedName: "pi-plugins-other-x",
          sourcePath: "/orig/x.md",
          targetPath: path.join(locations.agentsDir, "pi-plugins-other-x.md"),
          sourceHash: "abc",
          droppedFields: [],
          droppedTools: [],
          warnings: [],
...
test("unstagePluginAgents returns corruptions in warnings[] when index has per-row corruption", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    await writeFile(validTarget, makeOwnedFileContent("pi-plugins-acme-bot"));
    ...
    const onDisk = {
      ...
      agents: [
        ...
        // Missing required `targetPath`.
        {
          ...
          marketplace: "mp1",
          sourceAgent: "broken",
          generatedName: "pi-plugins-acme-broken",
