...
import { discoverPluginAgents } from "../../../extensions/pi-plugins/bridges/agents/discover.ts";
...
test("AG-1 generatedName for source 'bot' under plugin 'acme' is 'pi-plugins-acme-bot'", async () => {
  const { discovered: got } = await discoverPluginAgents({
    pluginName: "acme",
    agentsDirs: [TEST_PLUGIN_FIXTURE],
  });
  ...
  assert.ok(bot);
  assert.equal(bot.generatedName, "pi-plugins-acme-bot");
});

test("AG-1 generatedName elides plugin prefix: source 'acme-helper' under plugin 'acme' is 'pi-plugins-acme-helper'", async () => {
  const { discovered: got } = await discoverPluginAgents({
    pluginName: "acme",
    agentsDirs: [TEST_PLUGIN_FIXTURE],
  });
  ...
  // Result: 'pi-plugins-acme-helper' (NOT 'pi-plugins-acme-acme-helper').
  assert.equal(helper.generatedName, "pi-plugins-acme-helper");
});
...
test("D-07 discoverPluginAgents first-wins dedup across array elements (collision -> warning)", async () => {
  ...
  try {
    ...
    // name is `pi-plugins-p-shared`; first-wins keeps dir `a`.
    const a = path.join(dir, "a");
    const b = path.join(dir, "b");
    await mkdir(a, { recursive: true });
    await mkdir(b, { recursive: true });
    ...
    assert.match(warnings[0]!, /elides to generated name "pi-plugins-p-shared"/);
