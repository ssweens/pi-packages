...
} from "../../../extensions/pi-plugins/bridges/agents/stage.ts";
import { loadAgentsIndex } from "../../../extensions/pi-plugins/persistence/agents-index-io.ts";
import { locationsFor } from "../../../extensions/pi-plugins/persistence/locations.ts";
import { atomicWriteJson } from "../../../extensions/pi-plugins/shared/atomic-json.ts";
import { AgentOwnershipConflictError } from "../../../extensions/pi-plugins/shared/errors-bridges.ts";
import { cleanupStaging, pathExists } from "../../../extensions/pi-plugins/shared/fs-utils.ts";

import type { ResolvedPluginInstallable } from "../../../extensions/pi-plugins/domain/resolver.ts";
import type { AgentsIndex } from "../../../extensions/pi-plugins/persistence/agents-index-schema.ts";
...
test("AG-1 commitPreparedAgents lands files at <scopeRoot>/agents/pi-plugins-<plugin>-<agent>.md (happy path)", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    const botStat = await stat(path.join(locations.agentsDir, "pi-plugins-acme-bot.md"));
    assert.ok(botStat.isFile());
    const helperStat = await stat(path.join(locations.agentsDir, "pi-plugins-acme-helper.md"));
...
test("AG-2 saveAgentsIndex called with schemaVersion:1 and 2 rows (happy path)", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    assert.deepEqual(names, ["pi-plugins-acme-bot", "pi-plugins-acme-helper"]);
  });
});
...
test("AG-3 re-staging plugin X in mp1 leaves plugin X in mp2 rows untouched", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    // fire. We use "pi-plugins-acme-mp2only" -- a name that mp1's
    // staging will not produce.
    const seedTarget = path.join(locations.agentsDir, "pi-plugins-acme-mp2only.md");
      ...
      agents: [
        {
          ...
          marketplace: "mp2",
          sourceAgent: "mp2only",
          generatedName: "pi-plugins-acme-mp2only",
    ...
    assert.ok(mp2Row);
    assert.equal(mp2Row.generatedName, "pi-plugins-acme-mp2only");
  });
});
...
test("AG-5 prepare SURFACES foreign content via result.failed[] when previous targetPath has wrong basename", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    // (no pi-plugins- prefix). The file exists on disk.
      ...
      agents: [
        {
          ...
          marketplace: "mp1",
          sourceAgent: "ghost",
          generatedName: "pi-plugins-acme-ghost",
...
test("AG-5 prepare SURFACES foreign content via result.failed[] when previous targetPath body lacks marker", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    await mkdir(locations.agentsDir, { recursive: true });
    const fakeMineTarget = path.join(locations.agentsDir, "pi-plugins-acme-ghost.md");
...
test("AG-5 commit preserves foreign target on disk byte-identical when previous entry was AG-5-foreign", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    await mkdir(locations.agentsDir, { recursive: true });
    const foreignTarget = path.join(locations.agentsDir, "pi-plugins-acme-foreign.md");
    const foreignBytes = "---\nname: foreign\n---\nthis is not ours\n";
      ...
      agents: [
        {
          ...
          marketplace: "mp1",
          sourceAgent: "foreign",
          generatedName: "pi-plugins-acme-foreign",
...
test("AG-5 commit preserves foreign-content index row in agents:", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    const loaded = await loadAgentsIndex(locations);
    const preserved = loaded.agents.find((a) => a.generatedName === "pi-plugins-acme-foreign");
...
test("Phase 8 / PRL-10 replacePreparedAgents can rollback files and agents-index", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    const targetPath = path.join(locations.agentsDir, "pi-plugins-acme-bot.md");
    const oldBytes = "old generated by pi-plugins bytes";
  ...
  await withTmpScope(async ({ locations }) => {
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        ...
        assert.match(err.message, /pi-plugins-acme-foreign/);
...
test("AG-9/RN-4 prepare throws AgentOwnershipConflictError on cross-(mp,plugin) generated-name collision", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    // Pre-seed: another (mp2, rival) ALREADY owns "pi-plugins-acme-bot".
    const seed: AgentsIndex = {
      schemaVersion: 1,
      agents: [
        {
          ...
          generatedName: "pi-plugins-acme-bot",
          sourcePath: "/orig/rival.md",
          targetPath: path.join(locations.agentsDir, "pi-plugins-acme-bot.md"),
          sourceHash: "abc",
          droppedFields: [],
          droppedTools: [],
          warnings: [],
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        ...
        assert.equal(err.conflicts[0]?.generatedName, "pi-plugins-acme-bot");
...
test("AG-7 staged file content includes substituted ${CLAUDE_PLUGIN_ROOT}", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    const botContent = await readFile(
      path.join(locations.agentsDir, "pi-plugins-acme-bot.md"),
      "utf8",
    );
...
test("AG-5 staged file basename starts with pi-plugins-", async () => {
  await withTmpScope(async ({ locations }) => {
    const pluginRoot = path.join(FIXTURES, "test-plugin");
    const resolved = makeResolved("acme", pluginRoot);
    ...
    for (const name of prepared.result.stagedNames) {
      assert.ok(name.startsWith("pi-plugins-"), `name ${name} missing prefix`);
    }
  });
});
...
test("AG-5 staged file body contains GENERATED_AGENT_MARKER substring", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    for (const name of prepared.result.stagedNames) {
      ...
      assert.ok(content.includes("generated by pi-plugins"));
    }
  });
});
...
test("commit re-stage path: rm previous targets first, then rename staged", async () => {
  await withTmpScope(async ({ locations }) => {
    ...
    assert.ok(firstContent.includes("generated by pi-plugins"));
    ...
    assert.ok(secondContent.includes("generated by pi-plugins"));
  });
});
...
test("AG-1 prepare/replace skips backup loop entries that vanish between prepare and replace", async (t) => {
  ...
  await withTmpScope(async ({ locations }) => {
    ...
    const previousTarget = path.join(locations.agentsDir, "pi-plugins-acme-bot.md");
