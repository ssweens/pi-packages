...
import {
  ...
  partitionByOwner,
} from "../../../extensions/pi-plugins/bridges/agents/index-mutation.ts";

import type { AgentsIndexEntry } from "../../../extensions/pi-plugins/persistence/agents-index-schema.ts";
...
function makeEntry(overrides: Partial<AgentsIndexEntry> = {}): AgentsIndexEntry {
  return {
    ...
    generatedName: overrides.generatedName ?? "pi-plugins-acme-bot",
...
test("AG-9 findOwnershipConflicts returns single-name conflict", () => {
  ...
  const conflicts = findOwnershipConflicts([owner], ["pi-plugins-acme-bot"]);
  assert.equal(conflicts.length, 1);
  assert.equal(conflicts[0]?.generatedName, "pi-plugins-acme-bot");
