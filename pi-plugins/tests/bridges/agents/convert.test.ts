...
import {
  ...
  THINKING_VALUES,
  TOOL_MAP,
} from "../../../extensions/pi-plugins/bridges/agents/convert.ts";

import type { DiscoveredAgent } from "../../../extensions/pi-plugins/bridges/agents/types.ts";
...
function makeDiscovered(overrides: Partial<DiscoveredAgent> = {}): DiscoveredAgent {
  ...
  const generatedName = overrides.generatedName ?? `pi-plugins-acme-${sourceName}`;
  return {
    sourceName,
    generatedName,
...
test("AG-12 assertNoAgentCollisions throws with both source names listed when two source names elide to same generated", () => {
  assert.throws(
    () => {
      assertNoAgentCollisions([
        { sourceName: "bot", generatedName: "pi-plugins-acme-bot" },
        { sourceName: "acme-bot", generatedName: "pi-plugins-acme-bot" },
      ]);
    },
    (err: unknown) => {
      assert.ok(err instanceof Error);
...
test("AG-12 assertNoAgentCollisions returns silently when no collisions", () => {
  assert.doesNotThrow(() => {
    assertNoAgentCollisions([
      { sourceName: "bot", generatedName: "pi-plugins-acme-bot" },
      { sourceName: "helper", generatedName: "pi-plugins-acme-helper" },
    ]);
  });
});
