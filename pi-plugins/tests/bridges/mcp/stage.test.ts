...
import { PI_PLUGINS_MARKER_KEY } from "../../../extensions/pi-plugins/bridges/mcp/marker.ts";
import {
  abortPreparedMcp,
  commitPreparedMcp,
...
} from "../../../extensions/pi-plugins/bridges/mcp/stage.ts";
import { locationsFor } from "../../../extensions/pi-plugins/persistence/locations.ts";
import { McpServerCollisionError } from "../../../extensions/pi-plugins/shared/errors-bridges.ts";
...
test("MC-5 prepareStageMcpServers stamps each new entry with _piPlugins marker", async () => {
  await withTmpScope(async ({ cwd, locations }) => {
    ...
    assert.deepEqual(stagedDoc.a?.[PI_PLUGINS_MARKER_KEY], {
      plugin: PLUGIN,
      marketplace: MP,
    });
    assert.deepEqual(stagedDoc.b?.[PI_PLUGINS_MARKER_KEY], {
      plugin: PLUGIN,
      marketplace: MP,
    });
...
test("MC-6 commitPreparedMcp writes mcp.json atomically with full merged doc", async () => {
  await withTmpScope(async ({ cwd, locations }) => {
    ...
    assert.deepEqual(onDisk.mcpServers.srv?.[PI_PLUGINS_MARKER_KEY], {
      plugin: PLUGIN,
      marketplace: MP,
    });
  });
    ...
    await writeFile(
      ...
      JSON.stringify({
        mcpServers: {
          old: {
            command: "x",
            [PI_PLUGINS_MARKER_KEY]: { plugin: PLUGIN, marketplace: MP },
          },
        },
      }),
      "utf8",
    ...
    await writeFile(
      ...
      JSON.stringify({
        mcpServers: {
          dup: {
            command: "x",
            [PI_PLUGINS_MARKER_KEY]: { plugin: "other", marketplace: MP },
          },
        },
      }),
      "utf8",
...
test("MC-4 prepareStageMcpServers ALLOWS self-replace within own scope", async () => {
  await withTmpScope(async ({ cwd, locations }) => {
    ...
    assert.deepEqual(stagedDoc.srv?.[PI_PLUGINS_MARKER_KEY], {
      plugin: PLUGIN,
      marketplace: MP,
    });
  });
