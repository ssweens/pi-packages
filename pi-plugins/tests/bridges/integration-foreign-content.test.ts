...
// basename no longer matches `pi-plugins-*`) is preserved BYTE-
...
describe("integration: foreign content preservation", () => {
  ...
  // The basename matches `pi-plugins-acme-orphan` so AG-5's basename
  ...
  before(async () => {
    ...
    // fixture corpus.
    orphanTarget = path.join(locations.agentsDir, "pi-plugins-acme-orphan.md");
      ...
      agents: [
        {
          ...
          sourceAgent: "orphan",
          generatedName: "pi-plugins-acme-orphan",
  ...
  test("AG-5: foreign target with valid basename but missing marker is preserved byte-identical and surfaced via failed[]", async () => {
    ...
    assert.ok(
      !beforeBytes.includes("generated by pi-plugins"),
    ...
    const orphanRow = ourEntries.find((e) => e.generatedName === "pi-plugins-acme-orphan");
