...
} from "../../extensions/pi-plugins/bridges/agents/index.ts";
import {
  commitPreparedCommands,
  prepareStageCommands,
} from "../../extensions/pi-plugins/bridges/commands/index.ts";
import {
  commitPreparedMcp,
  prepareStageMcpServers,
  resolvePluginMcpServers,
} from "../../extensions/pi-plugins/bridges/mcp/index.ts";
import {
  commitPreparedSkills,
  prepareStageSkills,
} from "../../extensions/pi-plugins/bridges/skills/index.ts";
...
describe("integration: full-plugin staging", () => {
  ...
  test("AG-1/AG-2/AG-3/AG-5: agents bridge stages with marker discipline, partitions index by (mp, plugin)", async () => {
    ...
    // AG-1: every staged file basename starts with pi-plugins-.
    ...
    for (const name of agentsDirContents) {
      assert.match(
        name,
        /^pi-plugins-/,
        `AG-1: agent file basename "${name}" starts with pi-plugins-`,
      );
    }
    ...
    for (const name of agentsDirContents) {
      ...
      assert.ok(
        body.includes("generated by pi-plugins"),
    ...
    // AG-1 elision: bot -> pi-plugins-acme-bot (prefix added);
    // acme-helper -> pi-plugins-acme-helper (no double-prefix).
    assert.deepEqual(
      recordedNames,
      ["pi-plugins-acme-bot", "pi-plugins-acme-helper"],
      "AG-1: prefix added with no double-prefix",
    );
  });

  test("MC-5/MC-6: mcp bridge merges declared servers into mcp.json with _piPlugins marker", async () => {
