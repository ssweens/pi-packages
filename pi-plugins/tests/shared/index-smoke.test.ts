...
import claudeMarketplaceExtension from "../../extensions/pi-plugins/index.ts";
import { cleanupStaging } from "../../extensions/pi-plugins/shared/fs-utils.ts";

import type { ExtensionAPI } from "../../extensions/pi-plugins/platform/pi-api.ts";
...
test("registers command, read-only tools, session_start, and resources_discover exactly once", () => {
  ...
  assert.deepEqual(tools.map((e) => e.name).sort(), ["pi_plugins_list", "pi_plugins_plugin_list"]);
});
...
test("resources_discover handler resolves project cwd at invocation time", async () => {
  ...
  try {
    const projectPromptDir = path.join(eventCwd, ".pi", "pi-plugins", "resources", "prompts");
    ...
    const wrongPromptDir = path.join(processCwd, ".pi", "pi-plugins", "resources", "prompts");
    ...
    assert.ok(
      promptPaths.some((promptPath) =>
        promptPath.endsWith(
          path.join(".pi", "pi-plugins", "resources", "prompts", "cwd-captured.md"),
        ),
      ),
