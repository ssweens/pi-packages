...
import {
  ...
  generatedSkillName,
} from "../../extensions/pi-plugins/domain/name.ts";
...
test("RN-2 assertSafeName accepts long pi-namespaced agent name", () => {
  assert.doesNotThrow(() => {
    assertSafeName("pi-plugins-acme-bot");
  });
});
...
// ──────────────────────────────────────────────────────────────────────────
// RN-1 / AG-1: generatedAgentName -- "pi-plugins-<plugin>-<agent>"
// ──────────────────────────────────────────────────────────────────────────

test("AG-1 generatedAgentName basic case", () => {
  assert.equal(generatedAgentName("acme", "bot"), "pi-plugins-acme-bot");
});

test("AG-1 generatedAgentName elides plugin- prefix from source (Pitfall 8)", () => {
  assert.equal(generatedAgentName("acme", "acme-bot"), "pi-plugins-acme-bot");
});

test("AG-1 generatedAgentName always starts with pi-plugins- (AG-5 marker discipline)", () => {
  const result = generatedAgentName("acme", "bot");
  assert.ok(result.startsWith("pi-plugins-"));
});
