...
import { removeMarketplace } from "../../../extensions/pi-plugins/orchestrators/marketplace/remove.ts";
...
import { MarketplaceNotFoundError } from "../../../extensions/pi-plugins/shared/errors.ts";
...
test("NFR-5: remove for a path-source marketplace makes no network calls", async () => {
  ...
  const src = await readFile("extensions/pi-plugins/orchestrators/marketplace/remove.ts", "utf8");
