...
import { listMarketplaces } from "../../../extensions/pi-plugins/orchestrators/marketplace/list.ts";
...
test("ML-3: list source has zero imports from domain/manifest (no manifest reads)", async () => {
  const src = await readFile("extensions/pi-plugins/orchestrators/marketplace/list.ts", "utf8");
