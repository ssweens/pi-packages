...
import { setMarketplaceAutoupdate } from "../../../extensions/pi-plugins/orchestrators/marketplace/autoupdate.ts";
...
test("single-name cross-scope flip surfaces state lock failures instead of reporting partial success", async () => {
  await withHermeticHome(async ({ cwd }) => {
    ...
    try {
      ...
      assert.match(notifications[0]!.message, /Another pi-plugins operation is in progress/);
    } finally {
      await release();
    }
  });
