...
} from "../../extensions/pi-plugins/orchestrators/discover.ts";
import { AggregateResourcesDiscoverError } from "../../extensions/pi-plugins/shared/errors.ts";
...
function makeLocations(scope: Scope, root: string): ScopedLocations {
  const extensionRoot = path.join(root, "pi-plugins");
  return {
    scope,
    scopeRoot: root,
    extensionRoot,
