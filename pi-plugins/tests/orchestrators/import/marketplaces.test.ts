...
} from "../../../extensions/pi-plugins/orchestrators/import/index.ts";
...
test("import foundation modules stay pure and expose the Phase 10 API", async () => {
  ...
  for (const moduleName of moduleNames) {
    const source = await readFile(
      new URL(`../../../extensions/pi-plugins/orchestrators/import/${moduleName}`, import.meta.url),
      "utf8",
    );
  ...
  const barrel = await readFile(
    new URL("../../../extensions/pi-plugins/orchestrators/import/index.ts", import.meta.url),
    "utf8",
  );
