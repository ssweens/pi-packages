...
} from "../../../extensions/pi-plugins/orchestrators/plugin/shared.ts";
...
test("PI-6 / D-05 case C: skill + command + agent collisions -> deterministic order (skills, commands, agents; alpha within kind)", () => {
      ...
      plugins: {
        owner: makePluginRecord({
          resources: {
            ...
            agents: ["pi-plugins-x-agent", "pi-plugins-y-agent"],
            mcpServers: [],
          },
        }),
      },
  ...
  const names: CrossPluginGeneratedNames = {
    ...
    agents: ["pi-plugins-y-agent", "pi-plugins-x-agent"],
  };
  let captured: unknown;
  try {
    assertNoCrossPluginConflicts("user", names, state);
  ...
  assert.deepEqual(captured.conflicts, [
    ...
    `agent "pi-plugins-x-agent" already owned by plugin "owner"`,
    `agent "pi-plugins-y-agent" already owned by plugin "owner"`,
  ]);
});
