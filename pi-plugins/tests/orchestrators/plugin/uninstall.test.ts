...
import { cascadeUnstagePlugin } from "../../../extensions/pi-plugins/orchestrators/marketplace/shared.ts";
import { uninstallPlugin } from "../../../extensions/pi-plugins/orchestrators/plugin/uninstall.ts";
...
import { loadState, saveState } from "../../../extensions/pi-plugins/persistence/state-io.ts";
...
import type { ExtensionState } from "../../../extensions/pi-plugins/persistence/state-io.ts";
...
test("NFR-5: uninstall.ts has zero git surface (no platform/git, no DEFAULT_GIT_OPS, no gitOps)", async () => {
  const src = await readFile("extensions/pi-plugins/orchestrators/plugin/uninstall.ts", "utf8");
