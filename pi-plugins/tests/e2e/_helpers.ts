...
import type {
  ExtensionAPI,
  ExtensionContext,
} from "../../extensions/pi-plugins/platform/pi-api.ts";
...
async function prepareUpstreamCheckout(root: string): Promise<string> {
  ...
  const ref = process.env.PI_PLUGINS_E2E_REF === "main" ? "main" : PINNED_SHA;
...
export async function runPiRuntimeSmoke(): Promise<{
  ...
  const extension = path.join(REPO_ROOT, "extensions", "pi-plugins", "index.ts");
