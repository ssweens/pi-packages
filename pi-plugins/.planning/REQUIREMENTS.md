...
> **Source of truth:** All requirement IDs and full text live in `docs/prd/pi-plugins-prd.md`. This file preserves the PRD's IDs verbatim so existing references stay valid; one-line summaries here are excerpts, not redefinitions. When a summary and the PRD disagree, the PRD wins.
...
- [ ] **SK-1**: Skills staged at `<scope>/pi-plugins/resources/skills/<plugin>-<skill>/SKILL.md` with full directory copy
...
- [ ] **CM-1**: Commands staged as `<scope>/pi-plugins/resources/prompts/<plugin>:<command>.md`
...
- [ ] **AG-1**: Agent files staged at `<scope>/agents/pi-plugins-<plugin>-<agent>.md` (outside extension's `resources/`)
...
- [ ] **AG-5**: Generated agent files MUST start with `pi-plugins-` AND contain literal marker `generated by pi-plugins` in HTML-comment block after closing `---`
...
- [ ] **MC-5**: Each staged entry carries `_piPlugins: { plugin, marketplace }` marker
...
- [ ] **SC-2**: Extension data at `<scopeRoot>/pi-plugins/`; bridge files at `<scopeRoot>/agents/` and `<scopeRoot>/mcp.json`
...
- [ ] **RN-1**: Generated names deterministic from `(plugin, source-name)`. Skill: `<plugin>-<skill>` (prefix elided). Command: `<plugin>:<command>` (prefix elided). Agent: `pi-plugins-<plugin>-<agent>` (with `<plugin>-` prefix on source elided)
...
- [ ] **NFR-10**: System MUST refuse to write outside `<scopeRoot>/pi-plugins/`, `<scopeRoot>/agents/`, or `<scopeRoot>/mcp.json`
