...
must_haves:
  ...
  artifacts:
    - path: extensions/pi-plugins/shared/completion-cache.ts
    ...
    - path: extensions/pi-plugins/edge/completions/data.ts
    ...
    - path: extensions/pi-plugins/edge/completions/provider.ts
  ...
  key_links:
    - from: extensions/pi-plugins/edge/completions/data.ts
    ...
    - from: extensions/pi-plugins/edge/completions/data.ts
      to: extensions/pi-plugins/persistence/locations.ts
    ...
    - from: extensions/pi-plugins/shared/completion-cache.ts
      to: extensions/pi-plugins/shared/atomic-json.ts
...
<task type="auto" tdd="true">
  ...
  <files>extensions/pi-plugins/shared/completion-cache.ts, tests/shared/completion-cache.test.ts</files>
  <behavior>
...
- Schema constants `MARKETPLACE_NAMES_CACHE_SCHEMA` and `PLUGIN_INDEX_CACHE_SCHEMA` are exported so tests can snapshot `schemaVersion === 1`.
  ...
  <action>
1. Implement `extensions/pi-plugins/shared/completion-cache.ts` per the public surface above.
...
8. **ESLint check:** `shared/completion-cache.ts` MUST NOT import from `persistence/` or `domain/` (BLOCK C / shared-is-leaf rule). Verify with `grep`.
  ...
  <verify>
    <automated>node --test tests/shared/completion-cache.test.ts &amp;&amp; grep -v '^#' extensions/pi-plugins/shared/completion-cache.ts | grep -c "from \".*\\(persistence\\|domain\\|orchestrators\\|edge\\|bridges\\|presentation\\|transaction\\|platform\\)/" | grep -qx 0 &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/shared/completion-cache.ts tests/shared/completion-cache.test.ts</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>extensions/pi-plugins/edge/completions/data.ts, tests/edge/completions/data.test.ts</files>
  <behavior>
...
- TC-6 + D-03: install mode INCLUDES status `unavailable` rows (future --force).
  ...
  <action>
1. Port V1 pure helpers from `git show features/initial:extensions/pi-plugins/completions.ts` (the `buildItem`, `splitCompletionInput`, `extractPositionals`, `getScopeCompletions`, `getMarketplaceCompletions`, `getPluginCompletions` functions). They go in `data.ts` (per 06-PATTERNS.md, planner choice).
...
7. ESLint check: `edge/completions/data.ts` MUST NOT import from `persistence/`, `domain/`, `bridges/`, `transaction/`, `platform/`. ALLOWED: `shared/`, `orchestrators/` (but data.ts shouldn't need orchestrators either). Verify with grep.
  ...
  <verify>
    <automated>node --test tests/edge/completions/data.test.ts &amp;&amp; grep -v '^#' extensions/pi-plugins/edge/completions/data.ts | grep -c "from \".*\\(persistence\\|domain\\|bridges\\|transaction\\|platform\\)/" | grep -qx 0 &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/edge/completions/data.ts tests/edge/completions/data.test.ts</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>extensions/pi-plugins/edge/completions/provider.ts, tests/edge/completions/provider.test.ts</files>
  <behavior>
...
- TC-9: state.json error propagates from this function (handled by cache).
  ...
  <action>
1. Create `extensions/pi-plugins/edge/completions/provider.ts` implementing the dispatcher per 06-PATTERNS.md lines 188-247 and 06-RESEARCH.md lines 680-765:
...
4. Verify the import-boundary grep gate on `provider.ts`:
  ...
  <verify>
    <automated>node --test tests/edge/completions/provider.test.ts &amp;&amp; grep -v '^#' extensions/pi-plugins/edge/completions/provider.ts | grep -c "from \".*\\(persistence\\|domain\\|bridges\\|transaction\\|platform\\)/" | grep -qx 0 &amp;&amp; grep -n 'return null' extensions/pi-plugins/edge/completions/provider.ts | head -1 | grep -q 'return null' &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/edge/completions/provider.ts tests/edge/completions/provider.test.ts</automated>
  </verify>
