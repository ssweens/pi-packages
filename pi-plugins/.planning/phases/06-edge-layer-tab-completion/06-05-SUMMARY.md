...
provides:
  - "extensions/pi-plugins/orchestrators/edge-deps.ts: makeLocationsResolver(cwd) constructor that closes over persistence/state-io + persistence/locations + domain/manifest + domain/resolver. Returns LocationsResolverLike (a structural mirror of edge/completions/data.ts::LocationsResolver, kept locally because orchestrators/ MUST NOT import edge/ per BLOCK C). This resolves the planner's option (c) for the edge -> persistence indirection."
  - "extensions/pi-plugins/edge/register.ts: registerClaudePluginCommand(pi, deps) wires pi.registerCommand + pi.on(session_start). registerClaudeMarketplaceTools(pi) delegates to the two read-only LLM tools. Pitfall 3: process.cwd() at the registration glue layer is sanctioned exactly once."
...
key-files:
  created:
    ...
    - "extensions/pi-plugins/orchestrators/edge-deps.ts"
...
key-decisions:
  - "Task 3 (ESLint process.stdout/stderr block in edge/**) is a NO-OP. The existing BLOCK A rule in eslint.config.js already targets `extensions/pi-plugins/**/*.ts` -- a superset of `extensions/pi-plugins/edge/**/*.ts` -- and enumerates `process.stdout.write` + `process.stderr.write` as no-restricted-syntax errors. ROADMAP Phase 6 SC5 is already satisfied; the plan's Task 3 anticipated this outcome and authorized noting it in the SUMMARY rather than re-adding the rule."
...
|------|------|-----------------|
| `extensions/pi-plugins/orchestrators/edge-deps.ts` | LocationsResolver constructor closing over persistence + domain | `makeLocationsResolver(cwd)`, `LocationsResolverLike` |
| `extensions/pi-plugins/edge/register.ts` | D-04 registration glue | `registerClaudePluginCommand(pi, deps)`, `registerClaudeMarketplaceTools(pi)` |
...
| ROADMAP Phase 6 SC5 (process.stdout/stderr in edge/) | `eslint.config.js` (unchanged) | Existing BLOCK A rule already targets `extensions/pi-plugins/**` (superset); no diff needed. |
...
|--------|-------------------|--------|
| `extensions/pi-plugins/edge/register.ts` | persistence/, domain/, bridges/, transaction/, platform/ | `grep -nE` -> 0 matches |
| `extensions/pi-plugins/orchestrators/edge-deps.ts` | edge/, bridges/, transaction/, platform/ | `grep -nE` -> 0 matches |
...
1. **Task 3 is a no-op.** The existing ESLint BLOCK A rule
   (`extensions/pi-plugins/**/*.ts`) already enumerates
   ...
   ```text
   $ echo 'process.stdout.write("x");' > extensions/pi-plugins/edge/probe.ts
   $ npx eslint extensions/pi-plugins/edge/probe.ts
...
- **Fix:** Re-declared the resolver interface locally as
  ...
  compile or runtime.
- **Files affected:** `extensions/pi-plugins/orchestrators/edge-deps.ts`.
...
- **Files affected:** `extensions/pi-plugins/edge/register.ts`.
...
- extensions/pi-plugins/edge/register.ts -- FOUND
- extensions/pi-plugins/orchestrators/edge-deps.ts -- FOUND

All 10 modified files verified present (5 orchestrators + 5 test files):

- extensions/pi-plugins/orchestrators/marketplace/add.ts -- FOUND
- extensions/pi-plugins/orchestrators/marketplace/remove.ts -- FOUND
- extensions/pi-plugins/orchestrators/marketplace/update.ts -- FOUND
- extensions/pi-plugins/orchestrators/plugin/install.ts -- FOUND
- extensions/pi-plugins/orchestrators/plugin/uninstall.ts -- FOUND
