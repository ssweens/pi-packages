...
provides:
  ...
  - "orchestrators/plugin/list.ts: NEW export loadPluginListPayload(opts) -- pure payload builder extracted from listPlugins; listPlugins now delegates to it and only handles notify side-effects. tools.ts consumes this for pi_plugins_plugin_list execute body."
...
key-files:
  ...
  modified:
    ...
    - "extensions/pi-plugins/presentation/marketplace-list.ts"
...
2. **Task 2: 2 LLM tools (pi_plugins_list + plugin_list) + tools tests** -- `d5d00c3` (feat)
...
|--------|--------|---------|
| `loadVisibleMarketplaces({ cwd, scope? })` | `orchestrators/marketplace/shared.ts` | Returns `readonly { scope: Scope; record: MarketplaceRecord }[]` across the requested scope set (both scopes when undefined). Read-only; no notifications. Used by `pi_plugins_list` and the marketplace-existence check in `pi_plugins_plugin_list`. |
| `loadPluginListPayload(opts: ListPluginsOptions)` | `orchestrators/plugin/list.ts` | Returns `{ payload: PluginListPayload, warnings: readonly string[] }` for the same data `listPlugins` displays. Used by `pi_plugins_plugin_list` to project plugin rows for the LLM tool surface. `listPlugins` now delegates to it (no behavior change for the slash-command path). |
...
Comment-stripped grep gate over `extensions/pi-plugins/edge/handlers/**/*.ts` (10 files):
...
5. **`isError: true` only for TC-9 propagation** in `pi_plugins_plugin_list`. The marketplace-not-found surface (PL-1) returns plain text + empty `details.plugins` without `isError` to match V1 verbatim. Only an actual state.json load throw produces an `isError: true` response.
...
- **Found during:** Task 2, designing the `pi_plugins_plugin_list` execute body.
...
- **Files affected:** `extensions/pi-plugins/orchestrators/plugin/list.ts` only.
...
- **Fix:** Two added export lines at the top of `presentation/marketplace-list.ts`:
  ...
  ```
- **Files affected:** `extensions/pi-plugins/presentation/marketplace-list.ts` only.
...
- extensions/pi-plugins/edge/handlers/plugin/install.ts -- FOUND
- extensions/pi-plugins/edge/handlers/plugin/uninstall.ts -- FOUND
- extensions/pi-plugins/edge/handlers/plugin/update.ts -- FOUND
- extensions/pi-plugins/edge/handlers/plugin/list.ts -- FOUND
- extensions/pi-plugins/edge/handlers/marketplace/add.ts -- FOUND
- extensions/pi-plugins/edge/handlers/marketplace/remove.ts -- FOUND
- extensions/pi-plugins/edge/handlers/marketplace/list.ts -- FOUND
- extensions/pi-plugins/edge/handlers/marketplace/update.ts -- FOUND
- extensions/pi-plugins/edge/handlers/marketplace/autoupdate.ts -- FOUND
- extensions/pi-plugins/edge/handlers/tools.ts -- FOUND
