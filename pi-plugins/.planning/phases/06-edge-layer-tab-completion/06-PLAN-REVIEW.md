...
| SC5 | Every user-visible message via `ctx.ui.notify`; ESLint blocks `process.stdout`/`stderr` in `edge/` | 06-02 (notify-discipline grep gate), 06-04 (handlers gate), 06-05 (eslint.config.js edit) | Per-task grep gates + `npx eslint extensions/pi-plugins/edge/**/*.ts` |
