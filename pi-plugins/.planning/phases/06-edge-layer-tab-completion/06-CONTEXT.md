...
- `persistence/locations.ts` extension -- new helpers `cacheDir(loc)`, `marketplaceNamesCacheFile(loc)`, `pluginCacheFile(loc, marketplace)` returning paths under `<scopeRoot>/pi-plugins/cache/`
...
- **D-02 (V1's two read-only tools, with `pi_plugins_plugin_list` extended):**
  - `pi_plugins_list` -- V1 verbatim. Parameters: `Type.Object({})`. Returns one line per marketplace with `[<scope>] <name> -- <N> plugin(s) -- <source.logical>`. `details: { marketplaces }`.
  - `pi_plugins_plugin_list` -- V1 baseline + filter parameters. Parameters: `Type.Object({ marketplace?: string, scope?: "user"|"project", installed?: boolean, available?: boolean, unavailable?: boolean })`. When `marketplace` is omitted: enumerate across all marketplaces (V1 already does this). When any of `installed/available/unavailable` is set: PL-1 union semantics (no flag = all three; one or more flags = union of the named buckets). Status assignment matches PRD §5.3.1 PL-4 (`installed` from state.json `plugin.installed === true`; `available` from manifest entry where the plugin is installable and not installed; `unavailable` from manifest entry where the resolver says not-installable). Returns the same line-format and `details: { plugins: [...] }` shape as V1, refined to include filter-applied subset.
- **D-02 corollary (no mutating LLM tools):** PROJECT.md "Out of Scope" forbids `claude_install`, `claude_uninstall`, `claude_update`, `pi_plugins_add`, etc. The two tools above are the entire Phase 6 LLM surface.
...
- **D-03 (File-backed cache + in-memory layer; status-aware completion filtering):** Tab completion fires on every keystroke and must surface meaningful suggestions WITHOUT re-parsing `state.json` and every marketplace's `marketplace.json` per keystroke. Phase 6 introduces a two-tier cache:
  - **File-backed layer** (persistence; one file per cache):
    - `<scopeRoot>/pi-plugins/cache/marketplace-names.json` -- per scope. Schema: `{ schemaVersion: 1, names: string[] }`. Holds the union of marketplace names visible in that scope.
    - `<scopeRoot>/pi-plugins/cache/plugins/<marketplace>.json` -- per (scope, marketplace). Schema: `{ schemaVersion: 1, lastRefreshedAt: <iso>, manifestRef?: <sha-or-version>, plugins: [{ name, status: "installed" | "available" | "unavailable", version? }] }`. Holds the plugin index for completion.
...
- **D-03 corollary (cache file is OPTIMIZATION, not authoritative):** state.json and marketplace.json remain the authoritative sources. If a cache file is corrupted, missing, or deleted, completion still works (rebuild on miss). External tools/users can safely delete `<scopeRoot>/pi-plugins/cache/` -- it will be reconstructed lazily. This eliminates the migration / repair surface and keeps the cache schema versioning lightweight (schemaVersion 1; drop+rebuild on mismatch).
...
- **D-03 cache file naming:** `<scopeRoot>/pi-plugins/cache/marketplace-names.json` (singular path) and `<scopeRoot>/pi-plugins/cache/plugins/<marketplace>.json` (one per marketplace). The user did not specify exact paths; Claude chose these to match Phase 5 D-08's `data/` sibling pattern (`cache/` is a sibling of `data/`, `state.json`, `sources/`, etc. under `pi-plugins/`). Path containment goes through `assertPathInside` against `<scopeRoot>/pi-plugins/` (NFR-10 / Phase 1 D-14..17).
...
- `docs/prd/pi-plugins-prd.md` -- V1 PRD; authoritative spec for the successor
- `docs/prd/pi-plugins-prd.md` §6.6 -- AP-1..4 argument parsing rules (tokenizer quote semantics, `--scope` validation, Usage block, any-position flags)
- `docs/prd/pi-plugins-prd.md` §6.7 -- TC-1..9 tab completion rules (positional surfaces, `--scope` value list, plugin token form, manifest soft-fail, state error propagation, trailing space, whitespace normalization)
- `docs/prd/pi-plugins-prd.md` §5.1 / §5.2 / §5.3 -- subcommand semantics consumed by handlers (handlers must pass correct positional shapes to the orchestrators)
- `docs/prd/pi-plugins-prd.md` §6.2 -- SC-1 two-scope model; AP-2 `--scope user|project` exhaustive enumeration
- `docs/prd/pi-plugins-prd.md` §6.12 -- ES-1..5 error surfaces; AP-3 Usage block at `error` severity; IL-2 single notify channel (no `process.stdout`)
- `docs/prd/pi-plugins-prd.md` §10 -- NFR-5 network policy (completion data path MUST NOT touch network -- pure local file reads)
- `docs/prd/pi-plugins-prd.md` §11 -- V1 deferrals; `--force` install referenced by D-03 corollary (keep `unavailable` in install completions)
...
- `extensions/pi-plugins/shared/notify.ts` -- `notifySuccess/Warning/Error/UsageError`. Phase 6's `notifyUsageError` is the AP-3 Usage emission path
- `extensions/pi-plugins/shared/atomic-json.ts` -- `atomicWriteJson`. Phase 6's `shared/completion-cache.ts` writes go through this
- `extensions/pi-plugins/shared/path-safety.ts` -- `assertPathInside`. Every cache file path containment check
- `extensions/pi-plugins/shared/fs-utils.ts` -- `pathExists` (cache-miss detection without throwing)
- `extensions/pi-plugins/shared/errors.ts` -- `errorMessage(e)` (used by tokenizer to format `--scope` validation errors)
...
- `extensions/pi-plugins/domain/source.ts` -- consumed indirectly via marketplace records' `source` field rendering in `pi_plugins_list` tool output
- `extensions/pi-plugins/domain/manifest.ts` -- `loadMarketplaceManifest`. Phase 6's cache rebuild path consumes; per-marketplace manifest read failure soft-fails (TC-8)
- `extensions/pi-plugins/domain/resolver.ts` -- `resolveStrict`/`resolveLoose`. Plugin cache builds use these to compute the `unavailable` bucket (resolver returns `installable: false` for components/dependencies that can't be satisfied)
- `extensions/pi-plugins/persistence/locations.ts` -- `ScopedLocations` brand. Phase 6 ADDS helpers: `cacheDir(loc)`, `marketplaceNamesCacheFile(loc)`, `pluginCacheFile(loc, marketplace)` -- all returning paths under `<scopeRoot>/pi-plugins/cache/` with `assertPathInside` containment
- `extensions/pi-plugins/persistence/state-io.ts` -- `loadState`. TC-9 propagation surface; Phase 6's `getMarketplaceNames`/`getPluginIndex` rebuild paths consume
...
- `extensions/pi-plugins/orchestrators/marketplace/{add,remove,list,update,autoupdate}.ts` -- Phase 6 handlers wrap each one. Cache-invalidation calls ADDED post-state-commit per D-03 corollary
- `extensions/pi-plugins/orchestrators/marketplace/shared.ts` -- `GitOps`, `DEFAULT_GIT_OPS`. `EdgeDeps.gitOps` defaults to `DEFAULT_GIT_OPS` if Phase 7 doesn't override (Phase 7 will override with the live isomorphic-git wrapper)
- `extensions/pi-plugins/orchestrators/types.ts` -- `PluginUpdateFn`, `PluginUpdateOutcome`. `EdgeDeps.pluginUpdate` is typed by `PluginUpdateFn`
- `extensions/pi-plugins/presentation/{reload-hint,soft-dep,marketplace-list,plugin-list}.ts` -- pure renderers. Handlers do NOT consume these directly -- orchestrators do. Phase 6 just passes through to orchestrators
...
- `extensions/pi-plugins/orchestrators/plugin/{install,uninstall,update,list,shared}.ts` -- Phase 6 plugin handlers wrap each. Cache-invalidation calls ADDED post-state-commit per D-03 corollary
- `extensions/pi-plugins/orchestrators/plugin/index.ts` -- barrel; Phase 6 may import directly from per-verb files for clarity

### V1 reference (read selectively when implementing the same concern)

- `git show features/initial:extensions/pi-plugins/index.ts` -- V1 entrypoint; the registration shape Phase 6's `register.ts` replicates inside `registerClaudePluginCommand` (slash command + `session_start` autocomplete wrapper)
- `git show features/initial:extensions/pi-plugins/commands/router.ts` -- V1 router (`routeClaudePlugin` + `routeMarketplace`); Phase 6 ports verbatim with the new handler-shape signature
- `git show features/initial:extensions/pi-plugins/args.ts` -- V1 tokenizer + `parseArgs`; Phase 6 ports verbatim to `edge/args.ts` (AP-1, AP-2, AP-4)
- `git show features/initial:extensions/pi-plugins/commands/_args.ts` -- V1 `parseCommandArgs` schema validator; Phase 6 ports verbatim to `edge/args-schema.ts`
- `git show features/initial:extensions/pi-plugins/completions.ts` -- V1 completions; Phase 6 ports the helper functions (`buildItem`, `splitCompletionInput`, `extractPositionals`, `getPluginRefCompletions`, `normalizeCompletionWhitespace`, `isClaudePluginCommandLine`) and the `getArgumentCompletions` dispatcher logic. The data-loading functions (`loadKnownMarketplaceNames`, `loadAvailablePluginNames`, `loadInstalledPluginNames`, `loadPluginToMarketplacesMap`) are REPLACED by the cache layer (D-03)
- `git show features/initial:extensions/pi-plugins/commands/list-marketplaces.ts` -- V1 LLM tool registrations (`registerListMarketplacesTool`, `registerListPluginsTool`); Phase 6 ports verbatim to `edge/handlers/tools.ts` with the extended `pi_plugins_plugin_list` parameter schema (D-02)
- `git show features/initial:extensions/pi-plugins/commands/{install-plugin,uninstall-plugin,update,update-marketplace,marketplace-autoupdate,list,add-marketplace,remove-marketplace}.ts` -- V1 handler files; Phase 6 ports each as a thin shim per D-01 (handler signature changes from V1's monolithic to Phase 4/5 orchestrator delegation)
...
- **`extensions/pi-plugins/shared/notify.ts`** -- All Phase 6 user-visible messages: `notifyUsageError(ctx, usage)` for AP-3 Usage emission; `notifyError(ctx, msg, cause?)` for tokenizer/validation errors; `notifyWarning(ctx, msg)` for cache-invalidation failures (D-03 corollary)
- **`extensions/pi-plugins/shared/atomic-json.ts`** -- `atomicWriteJson(path, value)` for cache file writes
- **`extensions/pi-plugins/shared/path-safety.ts`** -- `assertPathInside(parent, child)` for cache path containment
- **`extensions/pi-plugins/shared/markers.ts`** -- Phase 6 does NOT add new markers. Usage strings are plain consts in `edge/router.ts` (stable but not contract-bound -- they can drift without breaking the user contract)
- **`extensions/pi-plugins/persistence/locations.ts`** -- Phase 6 ADDS `cacheDir(loc: ScopedLocations): string`, `marketplaceNamesCacheFile(loc: ScopedLocations): string`, `pluginCacheFile(loc: ScopedLocations, marketplace: string): string`. Each routes through `assertPathInside` against `<scopeRoot>/pi-plugins/`
- **`extensions/pi-plugins/persistence/state-io.ts`** -- `loadState(extensionRoot)`. Cache rebuild path; TC-9 propagation
- **`extensions/pi-plugins/domain/manifest.ts`** -- `loadMarketplaceManifest(source, options)`. Cache rebuild path; TC-8 soft-fail
- **`extensions/pi-plugins/domain/resolver.ts`** -- `resolveStrict(...)`. Used by cache build to compute plugin `status: "available" | "unavailable"`
- **`extensions/pi-plugins/orchestrators/marketplace/{add,remove,list,update,autoupdate}.ts`** -- Phase 6 handlers wrap each. Cache-invalidation calls ADDED at the end of each mutating orchestrator (post-state-commit) per D-03 corollary
- **`extensions/pi-plugins/orchestrators/marketplace/shared.ts`** -- `GitOps`, `DEFAULT_GIT_OPS`; `EdgeDeps.gitOps` defaults here
- **`extensions/pi-plugins/orchestrators/plugin/{install,uninstall,update,list}.ts`** -- Phase 6 handlers wrap each. Cache-invalidation calls ADDED to install/uninstall (update is a no-op for the current cache schema)
- **`extensions/pi-plugins/orchestrators/types.ts`** -- `PluginUpdateFn`, `PluginUpdateOutcome`; `EdgeDeps.pluginUpdate` is typed by `PluginUpdateFn`
- **`extensions/pi-plugins/presentation/{reload-hint,soft-dep,marketplace-list,plugin-list}.ts`** -- Phase 6 does NOT touch directly; orchestrators consume
...
- **LLM tool invocation:** `pi_plugins_list` and `pi_plugins_plugin_list` are invoked by the Pi LLM agent; they return `{ content: [{type: "text", text}], details: {...} }`. The agent sees the `text` field; the `details` field is for programmatic consumers (e.g., a future agent UI that renders structured data)
- **State recording:** Phase 6 does NOT directly mutate `state.json`. All state writes go through orchestrators. Cache files (`<scopeRoot>/pi-plugins/cache/`) are derived state -- they can be rebuilt from `state.json` + `marketplace.json` at any time
...
- **LLM tool test taxonomy** -- `tests/edge/handlers/tools.test.ts` covers both tools: (a) `pi_plugins_list` with empty state → "No marketplaces configured." + `details: { marketplaces: [] }`; (b) `pi_plugins_list` populated → one line per marketplace with `[<scope>] <name> -- <N> plugin(s) -- <source.logical>`; (c) `pi_plugins_plugin_list` with `marketplace` set, marketplace exists → plugins from that marketplace; (d) marketplace doesn't exist → `Marketplace "<name>" not found.` + empty `details.plugins`; (e) `marketplace` omitted → enumerate across all marketplaces; (f) filters `installed: true` → only installed bucket; (g) `available: true, unavailable: true` → both buckets unioned; (h) no filters → all three buckets (PL-1 semantics).
...
- **Top-level `claude_plugin_list` LLM tool (cross-marketplace native form)** -- Subsumed by D-02's extension to `pi_plugins_plugin_list` (which now supports omitted-marketplace + filter booleans). A future split into a dedicated `claude_plugin_list` tool with richer ergonomics is post-V1.
- **`pi_plugins_info`, `claude_plugin_info` tools** -- PRD §11 deferral (info subcommand). LLM-tool analogue is post-V1.
...
- **Cache UI / inspection command** -- A future `/claude:plugin cache list` or `/claude:plugin cache invalidate` would be useful for diagnosing stale completions. Not Phase 6; users can manually delete `<scopeRoot>/pi-plugins/cache/` (cache is optimization-only per D-03 corollary).
