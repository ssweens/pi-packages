...
**D-02 (Two read-only LLM tools, with `pi_plugins_plugin_list` extended):**
- `pi_plugins_list` -- V1 verbatim. Parameters: `Type.Object({})`. Returns `[<scope>] <name> -- <N> plugin(s) -- <source.logical>` per line. `details: { marketplaces }`.
- `pi_plugins_plugin_list` -- V1 baseline + filter parameters: `Type.Object({ marketplace?: string, scope?: "user"|"project", installed?: boolean, available?: boolean, unavailable?: boolean })`. Omitting `marketplace` enumerates all marketplaces. No-filter = all three buckets. PL-1 union semantics when any filter is set. Status assignment: `installed` from state-record presence (D-09 schema has no `installed` boolean -- presence ≡ installed); `available` from manifest entry where resolver is installable and the plugin is not in state; `unavailable` from manifest entry where resolver returns not-installable.

**D-02 corollary (No mutating LLM tools):** Two tools above are the entire Phase 6 LLM surface. No `claude_install`, `claude_uninstall`, `claude_update`, `pi_plugins_add`.
...
- File-backed layer:
  - `<scopeRoot>/pi-plugins/cache/marketplace-names.json` per scope. Schema: `{ schemaVersion: 1, names: string[] }`.
  - `<scopeRoot>/pi-plugins/cache/plugins/<marketplace>.json` per (scope, marketplace). Schema: `{ schemaVersion: 1, lastRefreshedAt: <iso>, manifestRef?: <sha-or-version>, plugins: [{ name, status: "installed" | "available" | "unavailable", version? }] }`.
...
**D-03 corollary (Cache is optimization, not authoritative):** Corrupt/missing cache → rebuild lazily. External tools may delete `<scopeRoot>/pi-plugins/cache/` safely. Schema mismatch → drop + rebuild.
...
- Cache file naming: `<scopeRoot>/pi-plugins/cache/marketplace-names.json` + `<scopeRoot>/pi-plugins/cache/plugins/<marketplace>.json` (chosen to match Phase 5 D-08 `data/` sibling pattern).
...
- Top-level `claude_plugin_list` (subsumed by extended `pi_plugins_plugin_list`).
- `pi_plugins_info`, `claude_plugin_info` tools.
...
- `pi_plugins_plugin_list` returning `version` for available plugins.
...
- **NFR-10 (containment):** All cache paths through `assertPathInside` against `<scopeRoot>/pi-plugins/`.
...
export async function getMarketplaceNames(
  marketplaceNamesCachePath: string,    // <scopeRoot>/pi-plugins/cache/marketplace-names.json
  scope: Scope,
...
### `extensions/pi-plugins/args.ts` (V1)
...
### `extensions/pi-plugins/commands/_args.ts` (V1)
...
### `extensions/pi-plugins/commands/router.ts` (V1)
...
### `extensions/pi-plugins/completions.ts` (V1)
...
### `extensions/pi-plugins/index.ts` (V1 -- dispatcher logic)
...
All signatures verified verbatim from `/Users/acolomba/src/pi-plugins/node_modules/@mariozechner/pi-coding-agent/dist/core/extensions/types.d.ts` (cross-referenced in §Architecture Patterns Pattern 3 above):
...
| `AutocompleteItem` | `{ value: string; label: string; description?: string }` (verified in `/Users/acolomba/src/pi-plugins/node_modules/@mariozechner/pi-tui/dist/autocomplete.d.ts`) | Every completion's return shape |
...
Verified in `/Users/acolomba/src/pi-plugins/eslint.config.js`:

```javascript
// BLOCK C: import-x/no-restricted-paths
{
  target: "./extensions/pi-plugins/edge",
  from: [
    "./extensions/pi-plugins/bridges",
    "./extensions/pi-plugins/domain",
    "./extensions/pi-plugins/transaction",
    "./extensions/pi-plugins/persistence",
    "./extensions/pi-plugins/platform",
  ],
...
**Output discipline (BLOCK A, also already in place):** Direct `process.stdout.write`, `process.stderr.write`, `console.*` calls and direct `ctx.ui.notify` are blocked across `extensions/pi-plugins/**/*.ts`. The single per-file override (`shared/notify.ts`) is the sanctioned chokepoint. **Phase 6 plan files MUST route every user-visible message through one of `notifySuccess`/`notifyWarning`/`notifyError`/`notifyUsageError`** -- same as Phases 4 and 5. ROADMAP SC5 explicitly says "ESLint blocks any new `process.stdout`/`stderr` write in `src/edge/`" -- this is already covered by BLOCK A applied to `extensions/pi-plugins/**`.
...
3. **Schema for filter parameters of `pi_plugins_plugin_list`:**
   ```typescript
   parameters: Type.Object({
     marketplace: Type.Optional(Type.String({ description: "Marketplace name to list plugins for" })),
     scope: Type.Optional(Type.Union(
...
4. **Schema for empty parameters of `pi_plugins_list`:**
   ```typescript
   parameters: Type.Object({}),
   ```
   Identical to V1.
...
- `/Users/acolomba/src/pi-plugins/.planning/phases/06-edge-layer-tab-completion/06-CONTEXT.md` -- locked decisions (D-01..D-04)
- `/Users/acolomba/src/pi-plugins/.planning/phases/06-edge-layer-tab-completion/06-DISCUSSION-LOG.md` -- decision audit trail
- `/Users/acolomba/src/pi-plugins/.planning/REQUIREMENTS.md` lines 200-217 -- AP-1..4, TC-1..9
- `/Users/acolomba/src/pi-plugins/.planning/ROADMAP.md` lines 143-152 -- Phase 6 entry
- `/Users/acolomba/src/pi-plugins/node_modules/@mariozechner/pi-coding-agent/dist/core/extensions/types.d.ts` -- Pi API surface (`ExtensionAPI.registerCommand`, `registerTool`, `on(session_start)`, `ExtensionContext.ui.notify`, `ExtensionContext.ui.addAutocompleteProvider`)
- `/Users/acolomba/src/pi-plugins/node_modules/@mariozechner/pi-tui/dist/autocomplete.d.ts` -- `AutocompleteItem`, `AutocompleteProvider`
- V1 source at `git show features/initial:extensions/pi-plugins/{args,index,completions}.ts` + `git show features/initial:extensions/pi-plugins/commands/{router,_args,list-marketplaces}.ts`
- `/Users/acolomba/src/pi-plugins/eslint.config.js` -- BLOCK A (output discipline) + BLOCK C (import boundaries) verified in-place
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/shared/{notify,atomic-json,path-safety,fs-utils,types}.ts` -- Phase 1 primitives Phase 6 consumes
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/persistence/{locations,state-io}.ts` -- Phase 2 primitives; cache rebuild consumes
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/{marketplace,plugin}/*.ts` -- Phase 4/5 orchestrators Phase 6 wraps + invalidation insertion sites
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/domain/{manifest,resolver}.ts` -- `MARKETPLACE_VALIDATOR`, `resolveStrict` consumed by cache rebuild

### Secondary (MEDIUM confidence)
- `/Users/acolomba/src/pi-plugins/.planning/research/PITFALLS.md` -- Pitfall 11 (cwd), 13 (completion staleness), 15 (notify discipline)
- `/Users/acolomba/src/pi-plugins/.planning/research/ARCHITECTURE.md` -- Pattern 6 (probe at decision time), Anti-Pattern 2 (cache `getAllTools`)
- `/Users/acolomba/src/pi-plugins/tests/orchestrators/plugin/install.test.ts` -- Phase 5 test scaffolding patterns (`withHermeticHome`, `makeCtx`)
- `/Users/acolomba/src/pi-plugins/tests/helpers/{git-mock,prd-extract}.ts` -- reusable helpers
