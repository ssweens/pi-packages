...
must_haves:
  ...
  artifacts:
    ...
    - path: extensions/pi-plugins/edge/types.ts
      provides: "EdgeDeps interface (D-04)"
      exports: ["EdgeDeps"]
    - path: extensions/pi-plugins/edge/completions/normalize.ts
    ...
    - path: extensions/pi-plugins/persistence/locations.ts
  ...
  key_links:
    - from: extensions/pi-plugins/edge/router.ts
      to: extensions/pi-plugins/shared/notify.ts
      via: "import { notifyUsageError } from \"../shared/notify.ts\""
      pattern: "notifyUsageError"
    - from: extensions/pi-plugins/edge/args-schema.ts
      to: extensions/pi-plugins/edge/args.ts
      via: "import { parseArgs } from \"./args.ts\""
      pattern: "from \"./args"
    - from: extensions/pi-plugins/persistence/locations.ts
      to: extensions/pi-plugins/shared/path-safety.ts
...
Output:
- 5 new files under `extensions/pi-plugins/edge/`
...
@extensions/pi-plugins/shared/errors.ts
@extensions/pi-plugins/shared/types.ts
@extensions/pi-plugins/shared/path-safety.ts
@extensions/pi-plugins/persistence/locations.ts

<!-- Orchestrator types that EdgeDeps imports -->
@extensions/pi-plugins/orchestrators/marketplace/shared.ts
@extensions/pi-plugins/orchestrators/types.ts
...
Run:
```bash
git show features/initial:extensions/pi-plugins/args.ts
git show features/initial:extensions/pi-plugins/commands/_args.ts
git show features/initial:extensions/pi-plugins/commands/router.ts
git show features/initial:extensions/pi-plugins/completions.ts | sed -n '1,310p'
```
...
<task type="auto">
  ...
  <files>extensions/pi-plugins/edge/args.ts, extensions/pi-plugins/edge/args-schema.ts, tests/edge/args.test.ts, tests/edge/args-schema.test.ts</files>
  <action>
1. Run `git show features/initial:extensions/pi-plugins/args.ts` and create `extensions/pi-plugins/edge/args.ts` with the V1 content verbatim, modulo the import: change `import type { Scope } from "./types.ts"` to `import type { Scope } from "../shared/types.ts"`.

2. Run `git show features/initial:extensions/pi-plugins/commands/_args.ts` and create `extensions/pi-plugins/edge/args-schema.ts` with the V1 content verbatim, modulo imports:
...
Use `notify` callbacks (the function injected as a parameter to `parseCommandArgs`) -- args-schema.ts itself does NOT import `notifySuccess`/`notifyError` from `shared/notify.ts`; the caller (handler) passes a closure that wraps `notifyError(ctx, ...)`.
  ...
  <verify>
    <automated>node --test tests/edge/args.test.ts tests/edge/args-schema.test.ts && npx tsc --noEmit && npx eslint extensions/pi-plugins/edge/args.ts extensions/pi-plugins/edge/args-schema.ts tests/edge/args.test.ts tests/edge/args-schema.test.ts</automated>
  </verify>
...
<task type="auto">
  ...
  <files>extensions/pi-plugins/edge/router.ts, extensions/pi-plugins/edge/types.ts, extensions/pi-plugins/edge/completions/normalize.ts, tests/edge/router.test.ts, tests/edge/completions/normalize.test.ts</files>
  <action>
1. Run `git show features/initial:extensions/pi-plugins/commands/router.ts` and create `extensions/pi-plugins/edge/router.ts` with two refinements:
...
2. Create `extensions/pi-plugins/edge/types.ts` with the `EdgeDeps` interface (and re-export `SubcommandHandlers` from `./router.ts` so consumers can `import type { EdgeDeps, SubcommandHandlers } from "./types.ts"` -- single import surface):
   ```typescript
...
3. Create `extensions/pi-plugins/edge/completions/normalize.ts` containing:
...
6. CRITICAL: Verify (via `grep -n 'ctx.ui.notify' extensions/pi-plugins/edge/router.ts`) that the router does NOT call `ctx.ui.notify` directly. Only `notifyUsageError`/`notifyError`/`notifyWarning`/`notifySuccess` are allowed. ESLint BLOCK A will block direct notify anyway, but the grep is faster feedback.
  </action>
  <verify>
    <automated>grep -v '^#' extensions/pi-plugins/edge/router.ts | grep -c 'ctx\.ui\.notify' | grep -qx 0 &amp;&amp; node --test tests/edge/router.test.ts tests/edge/completions/normalize.test.ts &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/edge/router.ts extensions/pi-plugins/edge/types.ts extensions/pi-plugins/edge/completions/normalize.ts tests/edge/router.test.ts tests/edge/completions/normalize.test.ts</automated>
  </verify>
...
<task type="auto">
  ...
  <files>extensions/pi-plugins/persistence/locations.ts</files>
  <action>
1. Read the existing `extensions/pi-plugins/persistence/locations.ts` to confirm the `ScopedLocations` interface and `locationsFor` function shape.
...
- `grep -v '^#' extensions/pi-plugins/edge/router.ts | grep -c 'ctx\.ui\.notify' | grep -qx 0` confirms zero direct notify calls (notify-discipline self-invariant).
