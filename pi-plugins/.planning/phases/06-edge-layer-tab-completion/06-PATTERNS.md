...
|---------------------|------|-----------|----------------|---------------|
| `edge/router.ts` | router | request-response | `git show features/initial:extensions/pi-plugins/commands/router.ts` | exact (V1 port verbatim, notify routing changes) |
| `edge/args.ts` | parser | transform | `git show features/initial:extensions/pi-plugins/args.ts` | exact (V1 port verbatim, import path only) |
| `edge/args-schema.ts` | parser | transform | `git show features/initial:extensions/pi-plugins/commands/_args.ts` | exact (V1 port verbatim, import paths) |
| `edge/types.ts` | types | -- | `orchestrators/types.ts` | role-match (cross-module types module) |
| `edge/register.ts` | registration | event-driven | `git show features/initial:extensions/pi-plugins/index.ts` (V1 entrypoint) | role-match (V1 was monolithic; new is two helpers) |
| `edge/completions/provider.ts` | completion dispatcher | request-response | `git show features/initial:extensions/pi-plugins/index.ts` (getArgumentCompletions block) | exact (port verbatim with cache-backed accessors) |
| `edge/completions/data.ts` | accessor | read-through | `git show features/initial:extensions/pi-plugins/completions.ts` (load* helpers) | role-match (V1 loaders replaced by cache reads) |
| `edge/completions/normalize.ts` | utility | transform | `git show features/initial:extensions/pi-plugins/completions.ts::normalizeCompletionWhitespace` | exact (V1 port verbatim) |
...
| `edge/handlers/tools.ts` | LLM tool registration | request-response | `git show features/initial:extensions/pi-plugins/commands/list-marketplaces.ts` | exact (V1 port + filter params) |
...
**Analog:** `git show features/initial:extensions/pi-plugins/commands/router.ts`
...
**Analog:** `git show features/initial:extensions/pi-plugins/args.ts`
...
**Analog:** `git show features/initial:extensions/pi-plugins/commands/_args.ts`
...
**Analog:** `git show features/initial:extensions/pi-plugins/index.ts` (the `getArgumentCompletions: async (prefix) => { ... }` block inside `pi.registerCommand`).
...
**Analog (replaced):** `git show features/initial:extensions/pi-plugins/completions.ts` (the four `load*` functions: `loadKnownMarketplaceNames`, `loadAvailablePluginNames`, `loadInstalledPluginNames`, `loadPluginToMarketplacesMap`).
...
**Analog:** `git show features/initial:extensions/pi-plugins/completions.ts::normalizeCompletionWhitespace` + `isClaudePluginCommandLine` + `CLAUDE_PLUGIN_LINE`.
...
**Analog:** `git show features/initial:extensions/pi-plugins/commands/list-marketplaces.ts` (V1's `registerListMarketplacesTool` + `registerListPluginsTool`).
...
**Analog:** `git show features/initial:extensions/pi-plugins/index.ts` (V1 entrypoint).
...
Then: invoke `registerListMarketplacesTool(pi)` → look up `registered.get("pi_plugins_list")` → call `.execute(...)` → assert return shape.
...
- `registerClaudeMarketplaceTools(pi)` → `tools` has both `"pi_plugins_list"` and `"pi_plugins_plugin_list"`.
...
**Source:** `extensions/pi-plugins/shared/notify.ts`
...
**Source:** `extensions/pi-plugins/shared/path-safety.ts::assertPathInside`
...
**Source:** `extensions/pi-plugins/shared/atomic-json.ts::atomicWriteJson`
...
**Source:** `extensions/pi-plugins/persistence/state-io.ts` lines 33-84
...
**Analog search scope:**
- `extensions/pi-plugins/edge/` (current state: minimal scaffold)
- `extensions/pi-plugins/orchestrators/{plugin,marketplace}/` (Phase 4/5 outputs)
- `extensions/pi-plugins/shared/` (Phase 1 outputs)
- `extensions/pi-plugins/persistence/` (Phase 2 outputs)
- `tests/orchestrators/`, `tests/shared/`, `tests/persistence/`
- V1 reference: `git show features/initial:extensions/pi-plugins/{args,index,completions}.ts` + `commands/{router,_args,list-marketplaces}.ts`
