...
|----------|----------|--------|---------|
| `extensions/pi-plugins/edge/args.ts` | Tokenizer + `--scope` validator (AP-1, AP-2, AP-4) | VERIFIED | 92 lines; quote-aware tokenize; throws on missing/invalid scope; while-loop for any-position |
| `extensions/pi-plugins/edge/args-schema.ts` | `parseCommandArgs(args, schema, notifyError)` | VERIFIED | 100 lines; positional schema with required/optional; routes errors to `notifyError` callback |
| `extensions/pi-plugins/edge/router.ts` | Dispatch + Usage blocks (AP-3) | VERIFIED | 141 lines; `routeClaudePlugin` + `routeMarketplace`; `TOP_LEVEL_USAGE` + `MARKETPLACE_USAGE` constants; `rm` alias case |
| `extensions/pi-plugins/edge/types.ts` | `EdgeDeps` interface | VERIFIED | Present (946B) |
| `extensions/pi-plugins/edge/completions/normalize.ts` | TC-7 normalization | VERIFIED | 49 lines; `normalizeCompletionWhitespace` + `isClaudePluginCommandLine` regex |
| `extensions/pi-plugins/edge/completions/data.ts` | Cache-backed data accessors | VERIFIED | 11K; uses `SCOPES` constant for cross-scope enumeration |
| `extensions/pi-plugins/edge/completions/provider.ts` | `getArgumentCompletions` dispatcher | VERIFIED | 183 lines; 5 branches (TC-1..TC-6); status-aware mode for install/uninstall/update |
| `extensions/pi-plugins/edge/handlers/plugin/{install,uninstall,update,list}.ts` | Thin shim handlers | VERIFIED | All 4 present (1.4K-2.5K each) |
| `extensions/pi-plugins/edge/handlers/marketplace/{add,remove,list,update,autoupdate}.ts` | Thin shim handlers | VERIFIED | All 5 present (1.1K-1.8K each) |
| `extensions/pi-plugins/edge/handlers/tools.ts` | Two LLM tool registrations | VERIFIED | TypeBox params schemas; `pi_plugins_list` (empty params) + `pi_plugins_plugin_list` (filter params) |
| `extensions/pi-plugins/edge/register.ts` | `registerClaudePluginCommand` + `registerClaudeMarketplaceTools` | VERIFIED | 129 lines; both helpers exported; `pi.on("session_start")` wrapper installs normalize; `process.cwd()` at one sanctioned site |
| `extensions/pi-plugins/orchestrators/edge-deps.ts` | `makeLocationsResolver` constructor | VERIFIED | Present (8.0K); closes over persistence + domain; wraps manifest errors in `ManifestSoftFailError` (TC-8); propagates state errors (TC-9) |
| `extensions/pi-plugins/shared/completion-cache.ts` | Two-tier cache | VERIFIED | Present (13K); exports `getMarketplaceNames`, `getPluginIndex`, `invalidateMarketplaceNames`, `invalidateMarketplaceCache`, `dropMarketplaceCache`, `ManifestSoftFailError`, `__resetCacheForTests` |
| `extensions/pi-plugins/persistence/locations.ts` | Cache helpers added | VERIFIED | Lines 73-99, 130-208: `cacheDir`, `marketplaceNamesCacheFile`, `pluginCacheFile(mp)` -- the last routes through `assertSafeName` + `assertPathInside` (NFR-10) |
...
| `handlers/tools.ts::pi_plugins_list` | `marketplaces` payload | `loadVisibleMarketplaces({ cwd: ctx.cwd })` | YES (live state.json read) | FLOWING |
| `handlers/tools.ts::pi_plugins_plugin_list` | `plugins` payload | `loadPluginListPayload` (orchestrator) | YES (live state + manifest read) | FLOWING |
...
| ESLint blocks `process.stdout.write` in edge/ | inspect `eslint.config.js:67-83` | Rule applies to `extensions/pi-plugins/**/*.ts` (superset includes edge/); `process.stdout.write` and `process.stderr.write` in `no-restricted-syntax` array | PASS |
...
2. **Real LLM-agent invocation of `pi_plugins_list` / `pi_plugins_plugin_list`** -- agent call-shape (param marshaling, chat rendering) is end-to-end. Same Phase 7-bound classification.
