...
1. **Plan-level grep gate (Task 2 verify):** `grep -v '^#' extensions/pi-plugins/edge/router.ts | grep -c 'ctx\.ui\.notify' | grep -qx 0`. Counts substring occurrences (not call-expressions) so it is conservative -- ANY mention of the literal, including in docstrings, trips the gate. Result: **0 occurrences after the docstring rewrite (see Deviation 1).**
2. **ESLint BLOCK A (`eslint.config.js`):** `no-restricted-syntax` AST selector `CallExpression[callee.property.name='notify'][callee.object.property.name='ui']` makes any actual call-expression on `*.ui.notify` an ESLint error in `extensions/pi-plugins/**/*.ts` outside the per-file BLOCK B override for `shared/notify.ts`. Result: **`npx eslint extensions/pi-plugins/edge/router.ts` reports 0 errors.**
...
- `extensions/pi-plugins/persistence/locations.ts` (cacheDir, marketplaceNamesCacheFile, pluginCacheFile additions)
...
- **Files affected:** `extensions/pi-plugins/edge/router.ts` only.
- **Verification:** `grep -v '^#' extensions/pi-plugins/edge/router.ts | grep -c 'ctx\.ui\.notify'` -> 0; `npx eslint .../router.ts` -> 0 errors.
...
- extensions/pi-plugins/edge/args.ts -- FOUND
- extensions/pi-plugins/edge/args-schema.ts -- FOUND
- extensions/pi-plugins/edge/router.ts -- FOUND
- extensions/pi-plugins/edge/types.ts -- FOUND
- extensions/pi-plugins/edge/completions/normalize.ts -- FOUND

Modified file extends ScopedLocations:

- extensions/pi-plugins/persistence/locations.ts: contains `cacheDir` (8 refs), `pluginCacheFile` (4 refs), `marketplaceNamesCacheFile` (3 refs) -- FOUND

Notify-discipline gate:

- `grep -v '^#' extensions/pi-plugins/edge/router.ts | grep -c 'ctx\.ui\.notify'` -> 0 -- PASS
