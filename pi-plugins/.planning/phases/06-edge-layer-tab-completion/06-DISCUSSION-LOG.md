...
| Option | Description | Selected |
|--------|-------------|----------|
| Ship V1's two verbatim (Recommended) | `pi_plugins_list` + `pi_plugins_plugin_list` as-is. | |
| V1's two + top-level `claude_plugin_list` | Adds a third tool mirroring `/claude:plugin list` with filters. | |
| Just `pi_plugins_list` | Smallest surface; drops V1's plugin list tool. | |
| Drop LLM tools from Phase 6 | Defer all tool registration to Phase 7. | |
| **Other (selected)** | Ship V1's two BUT extend `pi_plugins_plugin_list`: when marketplace name is omitted, list plugins in all marketplaces, filtered by `available/unavailable/installed` and `scope`. Unavailable plugins remain listable for install because a future `--force` flag will install their available components. | ✓ |

**User's choice:** Free-text -- V1's two tools, but `pi_plugins_plugin_list` becomes a hybrid: optional marketplace + filter booleans (installed/available/unavailable). PL-1 union semantics.
...
- Cache file naming and directory layout (Claude chose: `<scopeRoot>/pi-plugins/cache/marketplace-names.json` and `<scopeRoot>/pi-plugins/cache/plugins/<marketplace>.json`)
...
- Top-level `claude_plugin_list` LLM tool (subsumed by extended `pi_plugins_plugin_list`)
- `pi_plugins_info`, `claude_plugin_info` tools (PRD §11 info-subcommand deferral)
