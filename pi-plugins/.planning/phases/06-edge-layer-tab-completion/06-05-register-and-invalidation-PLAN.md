...
depends_on:
  ...
  - 06-04
files_modified:
  - extensions/pi-plugins/edge/register.ts
...
must_haves:
  ...
  artifacts:
    - path: extensions/pi-plugins/edge/register.ts
    ...
    - path: eslint.config.js
      provides: "no-restricted-syntax rule blocking direct process.stdout/stderr writes inside extensions/pi-plugins/edge/**"
      contains: "no-restricted-syntax"
  key_links:
    - from: extensions/pi-plugins/edge/register.ts
      to: extensions/pi-plugins/edge/handlers/plugin/install.ts
      via: "makeInstallHandler factory call"
      pattern: "makeInstallHandler"
    - from: extensions/pi-plugins/edge/register.ts
      to: extensions/pi-plugins/edge/completions/provider.ts
      via: "getArgumentCompletions wiring via LocationsResolver"
      pattern: "getArgumentCompletions"
    - from: extensions/pi-plugins/orchestrators/marketplace/add.ts
      to: extensions/pi-plugins/shared/completion-cache.ts
      via: "invalidateMarketplaceNames + invalidateMarketplaceCache (post-state-commit)"
      pattern: "invalidateMarketplace"
    - from: extensions/pi-plugins/orchestrators/plugin/install.ts
      to: extensions/pi-plugins/shared/completion-cache.ts
...
3. ESLint rule update: add a `no-restricted-syntax` (or per-file `no-restricted-imports`) rule blocking `process.stdout`/`process.stderr` direct writes inside `extensions/pi-plugins/edge/**` (ROADMAP Phase 6 SC5; the existing BLOCK A already covers `console.*` and direct `ctx.ui.notify`; this extends to the two `process.*` write surfaces).
...
@extensions/pi-plugins/shared/completion-cache.ts
@extensions/pi-plugins/edge/completions/provider.ts
@extensions/pi-plugins/edge/completions/data.ts
@extensions/pi-plugins/edge/completions/normalize.ts
@extensions/pi-plugins/edge/router.ts
...
- Run: git show features/initial:extensions/pi-plugins/index.ts
...
<task type="auto" tdd="true">
  ...
  <files>extensions/pi-plugins/orchestrators/marketplace/add.ts, extensions/pi-plugins/orchestrators/marketplace/remove.ts, extensions/pi-plugins/orchestrators/marketplace/update.ts, extensions/pi-plugins/orchestrators/plugin/install.ts, extensions/pi-plugins/orchestrators/plugin/uninstall.ts, tests/orchestrators/marketplace/add.test.ts, tests/orchestrators/marketplace/remove.test.ts, tests/orchestrators/marketplace/update.test.ts, tests/orchestrators/plugin/install.test.ts, tests/orchestrators/plugin/uninstall.test.ts</files>
  <behavior>
...
5. **Notify discipline:** the orchestrator files already use `notifyWarning` (Phase 4/5 carry-forward). No new direct `ctx.ui.notify` calls. Verify with grep on each modified orchestrator.
  ...
  <verify>
    <automated>node --test "tests/orchestrators/marketplace/add.test.ts" "tests/orchestrators/marketplace/remove.test.ts" "tests/orchestrators/marketplace/update.test.ts" "tests/orchestrators/plugin/install.test.ts" "tests/orchestrators/plugin/uninstall.test.ts" &amp;&amp; node -e 'const {execSync}=require("child_process"); const files=["extensions/pi-plugins/orchestrators/marketplace/add.ts","extensions/pi-plugins/orchestrators/marketplace/remove.ts","extensions/pi-plugins/orchestrators/marketplace/update.ts","extensions/pi-plugins/orchestrators/plugin/install.ts","extensions/pi-plugins/orchestrators/plugin/uninstall.ts"]; for(const f of files){const c=execSync(`grep -nE "invalidateMarketplaceCache\\\\|invalidateMarketplaceNames\\\\|dropMarketplaceCache" ${f} || true`).toString(); if(!c.trim()){console.error(`missing cache-invalidation call in ${f}`);process.exit(1)}} console.log("ok")' &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/orchestrators/marketplace/add.ts extensions/pi-plugins/orchestrators/marketplace/remove.ts extensions/pi-plugins/orchestrators/marketplace/update.ts extensions/pi-plugins/orchestrators/plugin/install.ts extensions/pi-plugins/orchestrators/plugin/uninstall.ts</automated>
  </verify>
...
<task type="auto" tdd="true">
  ...
  <files>extensions/pi-plugins/orchestrators/edge-deps.ts, extensions/pi-plugins/edge/register.ts, tests/edge/register.test.ts</files>
  <behavior>
...
- The wrapper applies normalizeCompletionWhitespace only when isClaudePluginCommandLine(originalLine) is true.
  ...
  <action>
1. Create `extensions/pi-plugins/orchestrators/edge-deps.ts` per the `<context>` shape. Verify the actual signatures of `locationsFor`, `loadState`, `loadMarketplaceManifest`, `resolveStrict` by reading them; adapt the closure bodies. The shape sketched in `<context>` may need adjustment based on Phase 2's actual exports.
...
2. Create `extensions/pi-plugins/edge/register.ts` per 06-PATTERNS.md lines 642-696:
...
5. **The import boundary for register.ts:**
  ...
  <verify>
    <automated>node --test tests/edge/register.test.ts &amp;&amp; node -e 'const {execSync}=require("child_process"); const out=execSync("grep -nE \"from \\\".*(persistence|domain|bridges|transaction|platform)/\" extensions/pi-plugins/edge/register.ts || true").toString(); if(out.trim()){console.error(`register.ts has forbidden imports:\\n${out}`);process.exit(1)} const out2=execSync("grep -nE \"ctx\\\\.ui\\\\.notify\" extensions/pi-plugins/edge/register.ts || true").toString(); if(out2.trim()){console.error(`direct notify in register.ts:\\n${out2}`);process.exit(2)} console.log("ok")' &amp;&amp; npx tsc --noEmit &amp;&amp; npx eslint extensions/pi-plugins/orchestrators/edge-deps.ts extensions/pi-plugins/edge/register.ts tests/edge/register.test.ts</automated>
  </verify>
...
2. Add a new rule targeting `extensions/pi-plugins/edge/**/*.ts` that blocks the following `MemberExpression` patterns:
...
4. Run `npm run check` to confirm no existing edge/ file regresses on this rule.
  ...
  <verify>
    <automated>npx eslint "extensions/pi-plugins/edge/**/*.ts" &amp;&amp; bash -c 'TMP=$(mktemp -d); echo "process.stdout.write(\"x\");" > $TMP/probe.ts; cp eslint.config.js $TMP/; cd $TMP &amp;&amp; npx eslint --no-eslintrc --config eslint.config.js probe.ts 2>&amp;1 | grep -q "process\\.stdout\\.write" &amp;&amp; echo "probe-blocked: ok" || echo "probe-blocked: missing rule -- check eslint.config.js"; rm -rf $TMP' &amp;&amp; npm run check</automated>
  </verify>
...
<verification>
- 1 new file: `extensions/pi-plugins/edge/register.ts`.
- 1 new file: `extensions/pi-plugins/orchestrators/edge-deps.ts`.
