...
requires:
  ...
  - phase: 01-foundations-toolchain/04
    provides: "extensions/pi-plugins/index.ts entrypoint that index-smoke test imports"
...
### Source code (`extensions/pi-plugins/`)
...
8. **`extensions/pi-plugins.ts` legacy stub is GONE.** Pi loads `extensions/pi-plugins/index.ts` via the directory entrypoint pattern. If you ever see references to the old single-file shape, they're stale.
