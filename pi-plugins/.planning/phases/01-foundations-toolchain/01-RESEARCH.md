...
- **D-04:** Phase 1 fully rewires `package.json`: update `pi.extensions` to `./extensions/pi-plugins/index.ts`, update test globs, bump `typebox` 1.1.34→1.1.38, `prettier` 3.6.2→3.8.3, `globals` 17.5.0→17.6.0. Phase 1 ends with `npm run check` actually green.
...
- **D-06:** ESLint `no-restricted-syntax` blocks `process.stdout.write`, `process.stderr.write`, `console.log/warn/error/info` calls in `extensions/pi-plugins/`. Sanctioned `eslint-disable-next-line` at the single `migrateLegacyMarketplaceRecords` callsite (per IL-3).
...
- **D-10:** **9-folder split** under `extensions/pi-plugins/`: `edge/`, `orchestrators/`, `bridges/`, `domain/`, `transaction/`, `persistence/`, `presentation/`, `platform/`, `shared/`. Direct children of the extension dir (no `src/` wrapper).
...
- **D-13:** Replace `extensions/pi-plugins.ts` (current stub) with `extensions/pi-plugins/index.ts`.
...
| **NFR-10** | System MUST refuse to write outside `<scopeRoot>/pi-plugins/`, `<scopeRoot>/agents/`, or `<scopeRoot>/mcp.json` | `assertPathInside` chokepoint (D-15); symlink-walk (D-14/D-16/D-17)                               |
...
| Pi extension entrypoint           | `extensions/pi-plugins/index.ts` | `edge/`        | Stub for now; Phase 6 fills it; Phase 1 wires the `pi.registerCommand` + `pi.on("resources_discover")` skeleton |
...
└──────────────────────────────┬───────────────────────────────────┘
                               ...
                               ▼
┌──────────────────────────────────────────────────────────────────┐
│  extensions/pi-plugins/index.ts                          │
...
│   reads docs/prd/pi-plugins-prd.md at runtime,        │
...
**What:** AST-selector-based bans on direct stdout/stderr/console writes inside `extensions/pi-plugins/`.
...
**Concrete `eslint.config.js` block (planner: copy-paste this into the existing flat config, scoped to `extensions/pi-plugins/`):**

```javascript
// eslint.config.js -- append to the existing flat-config array

{
  files: ["extensions/pi-plugins/**/*.ts"],
  rules: {
...
- The sanctioned `console.warn` callsite (IL-3) lives in `persistence/state-io.ts` (Phase 2). Phase 1 doesn't write that file; Phase 2's planner needs to know the disable-comment incantation:
  ...
  try {
    ...
    console.warn(`pi-plugins: failed to persist migrated state.json: ${errorMessage(err)}`);
  }
  ```
    ...
    "import-x/no-restricted-paths": [
      "error",
      {
        ...
        zones: [
          ...
          {
            target: "./extensions/pi-plugins/orchestrators",
            from: ["./extensions/pi-plugins/edge"],
          ...
          {
            target: "./extensions/pi-plugins/bridges",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/transaction",
              "./extensions/pi-plugins/presentation",
              "./extensions/pi-plugins/platform",
            ],
            message:
          ...
          {
            target: "./extensions/pi-plugins/domain",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
          ...
          {
            target: "./extensions/pi-plugins/transaction",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
          ...
          {
            target: "./extensions/pi-plugins/persistence",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
          ...
          {
            target: "./extensions/pi-plugins/presentation",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
          ...
          {
            target: "./extensions/pi-plugins/platform",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
          ...
          {
            target: "./extensions/pi-plugins/shared",
            from: [
              "./extensions/pi-plugins/edge",
              "./extensions/pi-plugins/orchestrators",
              "./extensions/pi-plugins/bridges",
...
**What:** Phase 1's `extensions/pi-plugins/index.ts` replaces the current stub. At Phase-1-end, it registers the `claude:plugin` command + `resources_discover` event handler, but both delegate to (currently empty) edge handlers. Goal is to have **a real Pi extension** that loads cleanly and notifies "not implemented yet" -- exactly mirroring what the current stub does, but in the new layout.
...
- `ExtensionAPI.registerTool(tool: ToolDefinition<TParams, TDetails, TState>)` -- used by Phase 6 for the LLM-callable list tools. **NOT used in Phase 1's index.ts** -- the current stub registers `pi_plugins_list` but Phase 1 dropping it is fine (Phase 6 will re-register from `edge/handlers/list.ts`).
...
export default function claudeMarketplaceExtension(pi: ExtensionAPI): void {
  ...
  // current stub's pi_plugins_list). Phase 1's index.ts deliberately
...
| **Live service config**                  | `package.json` `pi.extensions` array currently points at `./extensions/pi-plugins/index.ts` (which doesn't exist yet -- it points at the future location).                               | Phase 1 makes the pointer correct by creating `extensions/pi-plugins/index.ts`. **Net: pointer moves from broken → live.** |
...
**What goes wrong:** Current `package.json` `pi.extensions` already points at `./extensions/pi-plugins/index.ts` -- but that file does NOT exist on this branch. The current stub is `./extensions/pi-plugins.ts` (without the dir). Pi loads, fails to find the indicated file, and fails the extension-load step. **D-04 fixes this**, but if Phase 1 is split into multiple waves and the package.json edit lands BEFORE the directory + index.ts are created, `npm run check`'s test step will fail at extension-load.
...
3. **Does `index.ts` need to register `pi_plugins_list` LLM tool in Phase 1?**
   ...
   - What's unclear: Does "thin entrypoint that sets up `pi.registerTool`" mean Phase 1 registers a stub `pi_plugins_list` (mirroring the current stub), or does Phase 1 register NO tools and Phase 6 introduces them?
...
- **`@mariozechner/pi-coding-agent@0.73.1`** `dist/core/extensions/types.d.ts` -- read directly from `node_modules/` on 2026-05-09. Verified `ExtensionAPI`, `registerCommand`, `registerTool`, `on("resources_discover")`, `ctx.ui.notify(message, type?)` shape. Source: `/Users/acolomba/src/pi-plugins/node_modules/@mariozechner/pi-coding-agent/dist/core/extensions/types.d.ts` lines 769-923.
...
- **PRD `docs/prd/pi-plugins-prd.md`** §6.10 (PS-1..5), §6.11 (AS-1, AS-4, AS-5), §6.12 (ES-1..5), §6.13 (IL-1..5), §10 (NFR-1..12) -- read at lines 579-622 and 963-979 on 2026-05-09. Authoritative spec.
