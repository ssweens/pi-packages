...
provides:
  - 9 folders under extensions/pi-plugins/ (edge, orchestrators, bridges, domain, transaction, persistence, presentation, platform, shared)
...
tech-stack:
  ...
  patterns:
    - "9-folder layout under extensions/pi-plugins/ (D-10) with sibling-import discipline (D-11)"
...
key-files:
  created:
    - extensions/pi-plugins/edge/README.md
    - extensions/pi-plugins/edge/index.ts
    - extensions/pi-plugins/orchestrators/README.md
    - extensions/pi-plugins/orchestrators/index.ts
    - extensions/pi-plugins/bridges/README.md
    - extensions/pi-plugins/bridges/index.ts
    - extensions/pi-plugins/domain/README.md
    - extensions/pi-plugins/domain/index.ts
    - extensions/pi-plugins/transaction/README.md
    - extensions/pi-plugins/transaction/index.ts
    - extensions/pi-plugins/persistence/README.md
    - extensions/pi-plugins/persistence/index.ts
    - extensions/pi-plugins/presentation/README.md
    - extensions/pi-plugins/presentation/index.ts
    - extensions/pi-plugins/platform/README.md
    - extensions/pi-plugins/platform/git.ts
    - extensions/pi-plugins/shared/README.md
  modified: []
...
**9-folder layered scaffold under `extensions/pi-plugins/` with placeholder READMEs documenting D-11 import-direction zones, plus the `platform/git.ts` isomorphic-git wrapper (D-18/D-19/D-20) replacing V1's `execFile("git")` shell-out.**
...
- `extensions/pi-plugins/edge/README.md` - edge/ purpose, allowed imports (orchestrators/, presentation/, shared/), Phase 6 planned contents
- `extensions/pi-plugins/edge/index.ts` - placeholder `export {};` so the canary fixture can import without resolution failure
- `extensions/pi-plugins/orchestrators/README.md` - orchestrators/ purpose, allowed imports (bridges/, domain/, transaction/, persistence/, presentation/, platform/, shared/), Phase 4-5 planned contents
- `extensions/pi-plugins/orchestrators/index.ts` - placeholder
- `extensions/pi-plugins/bridges/README.md` - bridges/ purpose, allowed imports (domain/, persistence/, shared/) + cross-bridge prohibition, Phase 3 planned contents
- `extensions/pi-plugins/bridges/index.ts` - placeholder (canary fixture target)
- `extensions/pi-plugins/domain/README.md` - domain/ pure-logic discipline (`MUST NOT import upward`), shared/ as only sibling, Phase 2-3 planned contents, note about Scope type moving to shared/types.ts
- `extensions/pi-plugins/domain/index.ts` - placeholder
- `extensions/pi-plugins/transaction/README.md` - transaction/ purpose, allowed imports (persistence/, shared/), Phase 2 planned contents
- `extensions/pi-plugins/transaction/index.ts` - placeholder
- `extensions/pi-plugins/persistence/README.md` - persistence/ purpose, allowed imports (domain/, shared/), Phase 2 planned contents, note about IL-3 sanctioned `console.warn` site
- `extensions/pi-plugins/persistence/index.ts` - placeholder
- `extensions/pi-plugins/presentation/README.md` - presentation/ purpose, allowed imports (domain/, shared/), Phase 4-6 planned contents, note about MARKERS consumption
- `extensions/pi-plugins/presentation/index.ts` - placeholder
- `extensions/pi-plugins/platform/README.md` - platform/ purpose, allowed imports (shared/ only), Phase 1 git.ts marked `[x]`, Phase 7 pi-api.ts planned
- `extensions/pi-plugins/platform/git.ts` - 7-function isomorphic-git wrapper (154 lines) with 7 typed option interfaces; namespace import shape; spread-conditional optional handling
- `extensions/pi-plugins/shared/README.md` - shared/ pure-leaves discipline (`MUST NOT import from any other extension folder`), Plan 01-02 markers/errors/notify/atomic-json/path-safety marked `[x]`, Phase 2 types.ts (`Scope`) planned
...
- **Verification:** `grep -c '## Allowed Imports' extensions/pi-plugins/*/README.md | wc -l` → 9 (all 9 READMEs have the section). The semantic intent ("each README documents its allowed-imports zone") is satisfied. `import-x/no-restricted-paths` in `eslint.config.js` is the actual enforcement; the README phrasing is documentation.
- **Committed in:** Task 1 commit `90fbaaa`

**2. [Rule 3 - Blocking] Plan's smoke test `mktemp` placement caused ERR_MODULE_NOT_FOUND**

- **Found during:** Task 2 verification
- **Issue:** Plan's smoke harness writes a `.mjs` file via `mktemp -t` (which lands in `/tmp`) and uses `await import("./extensions/pi-plugins/platform/git.ts")` -- the relative specifier resolves against the script's directory (`/tmp`), not the project root. Node 26's strict ESM resolver fails with `ERR_MODULE_NOT_FOUND`.
...
- **Found during:** Task 2 verification
- **Issue:** Plan acceptance asserts e.g. `grep '^import \* as git from "isomorphic-git"$' extensions/pi-plugins/platform/git.ts` returns 1 line. The actual line ends with `;` (TypeScript convention; Prettier preserves), so the anchored regex returns 0 lines.
...
- **Verification:** `grep -F 'import * as git from "isomorphic-git"' extensions/pi-plugins/platform/git.ts` returns 1 line. Same for `import http from "isomorphic-git/http/node"` and `import * as fs from "node:fs"`. `npm run typecheck` and `npm run lint` exit 0, confirming the import shapes parse correctly.
