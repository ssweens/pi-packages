...
### Q3 -- Where do the 9 folders live relative to extensions/pi-plugins/?

| Option                                        | Description                                             | Selected |
| --------------------------------------------- | ------------------------------------------------------- | -------- |
| Directly inside extension dir                 | extensions/pi-plugins/{edge,orchestrators,...}/ | ✓        |
| Wrapped in extensions/pi-plugins/src/ | Adds one path level                                     |          |
| You decide                                    | Mirror V1: folders directly inside the extension dir    |          |

**User's choice:** Directly inside extension dir (Recommended) **Notes:** Replaces current stub `pi-plugins.ts` with a directory; entrypoint is `extensions/pi-plugins/index.ts`.
