...
|---------------------|------|-----------|----------------|---------------|
| `extensions/pi-plugins/index.ts` | entrypoint | event-driven (Pi API) | `features/initial:extensions/pi-plugins/index.ts` | exact (role) -- Phase 1 trims to skeleton |
| `extensions/pi-plugins/shared/atomic-json.ts` | utility | file-I/O (JSON write) | `features/initial:extensions/pi-plugins/fs-utils.ts` (atomicWriteJson) | role-match -- replace impl with `write-file-atomic` |
| `extensions/pi-plugins/shared/path-safety.ts` | utility | file-I/O (lstat walk) | NONE on `features/initial` (V1 had no symlink check); RESEARCH.md Pattern 2 is canonical | greenfield with embedded research code |
| `extensions/pi-plugins/shared/markers.ts` | constants module | request-response (read-only consts) | NONE on V1 (markers were inline literals in `presentation/reload-hint.ts` etc.); PRD §6.12 is the source of truth | greenfield (extracted from PRD) |
| `extensions/pi-plugins/shared/notify.ts` | utility (presentation seam) | request-response (sync wrapper) | NONE on V1 (callers used `ctx.ui.notify(...)` directly throughout); RESEARCH.md Pattern 7 is canonical | greenfield (new API surface) |
| `extensions/pi-plugins/shared/errors.ts` | utility (error helpers) | transform | `features/initial:extensions/pi-plugins/errors.ts` | exact (verbatim port) |
| `extensions/pi-plugins/platform/git.ts` | platform facade | request-response (network/IO) | `features/initial:extensions/pi-plugins/marketplace/git.ts` | role-match -- replace `execFile("git")` with `isomorphic-git` |
| `extensions/pi-plugins/{edge,orchestrators,bridges,domain,transaction,persistence,presentation,platform,shared}/README.md` | documentation | n/a | NONE on V1 (V1 had no folder READMEs) | greenfield -- 9 placeholder READMEs |
...
### `extensions/pi-plugins/index.ts` (entrypoint, event-driven)

**Analog:** `features/initial:extensions/pi-plugins/index.ts`
...
### `extensions/pi-plugins/shared/atomic-json.ts` (utility, file-I/O)

**Analog:** `features/initial:extensions/pi-plugins/fs-utils.ts` (the `atomicWriteJson` function specifically)
...
### `extensions/pi-plugins/shared/path-safety.ts` (utility, file-I/O)
...
### `extensions/pi-plugins/shared/markers.ts` (constants, request-response)

**Analog:** NONE on V1. V1 had marker strings INLINED at usage sites (e.g., `features/initial:extensions/pi-plugins/presentation/reload-hint.ts` line 16: `` ` Run /reload to ${verb} it.` ``). D-08 centralizes them.
...
// without updating docs/prd/pi-plugins-prd.md §6.12 in the same
...
### `extensions/pi-plugins/shared/notify.ts` (utility, request-response)
...
```javascript
{
  files: ["extensions/pi-plugins/shared/notify.ts"],
...
### `extensions/pi-plugins/shared/errors.ts` (utility, transform)

**Analog:** `features/initial:extensions/pi-plugins/errors.ts` (verbatim port).
...
**Divergences from V1:** **NONE**. Verbatim port. Only the file location moves: `extensions/pi-plugins/errors.ts` → `extensions/pi-plugins/shared/errors.ts`.
...
### `extensions/pi-plugins/platform/git.ts` (platform facade, request-response)

**Analog:** `features/initial:extensions/pi-plugins/marketplace/git.ts` (V1's CLI shell-out version).
...
Phase 6 lands the user-facing command surface: argument parsing for `/claude:plugin <subcommand>`, completion providers, the `pi_plugins_list` LLM tool, and the dispatch table that maps subcommands to orchestrators.
...
- [ ] `handlers/list.ts` -- `pi_plugins_list` LLM tool (Phase 6)
```
...
1. **Output-discipline block** scoped to `extensions/pi-plugins/**/*.ts` -- 6 `no-restricted-syntax` selectors covering `process.stdout.write`, `process.stderr.write`, `console.log/warn/error/info`, plus a 7th selector forbidding direct `ctx.ui.notify(` (D-07). Elevate `no-console` from `"warn"` to `"error"` for that scope.
2. **Import-boundary block** scoped to `extensions/pi-plugins/**/*.ts` -- `import-x/no-restricted-paths` with 9 zones (one per folder) per RESEARCH.md Pattern 4.
3. **Per-file overrides:**
   - `extensions/pi-plugins/shared/notify.ts` → `"no-restricted-syntax": "off"` (the wrapper IS the sanctioned site)
...
|-------|---------|----------------|
| `pi.extensions` | `["./extensions/pi-plugins/index.ts"]` | unchanged (already correct, but file does not exist yet -- Phase 1 makes it exist; per RESEARCH.md Pitfall #7) |
...
import * as markers from "../../extensions/pi-plugins/shared/markers.ts";

const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
const PRD_PATH = path.join(REPO_ROOT, "docs/prd/pi-plugins-prd.md");
...
import "../../../extensions/pi-plugins/bridges/index.ts";
export {};
```
...
import {
  ...
  // ...
} from "../../extensions/pi-plugins/agent/frontmatter.ts";
...
**Source:** `extensions/pi-plugins/shared/notify.ts` (NEW, per D-07).
...
**Source:** `extensions/pi-plugins/shared/atomic-json.ts` (NEW, per D-03).
...
**Source:** `extensions/pi-plugins/shared/path-safety.ts` (NEW, per D-14..D-17).
...
**Apply to:** all files under `extensions/pi-plugins/`.
...
import { /* SUT */ } from "../../extensions/pi-plugins/<path>.ts";
...
- `features/initial` branch (V1 source) -- `extensions/pi-plugins/{agent,commands,location,marketplace,mcp,plugin,presentation,resource,state,transaction}/` and `tests/{agent,commands,location,...}/`
- Current `features/initial-gsd` branch -- `extensions/pi-plugins.ts` (stub), `eslint.config.js`, `package.json`, `tsconfig.json`
- RESEARCH.md (HIGH-confidence patterns where V1 has no precedent)

**Files inspected (verified 2026-05-09 via `git show` or `Read`):**

- `features/initial:extensions/pi-plugins/index.ts` (V1 entrypoint, 200+ lines)
- `features/initial:extensions/pi-plugins/fs-utils.ts` (atomicWriteJson, pathExists, dirExists, cleanupStaging)
- `features/initial:extensions/pi-plugins/errors.ts` (errorMessage, appendLeakToError, appendLeaks -- verbatim port target)
- `features/initial:extensions/pi-plugins/marketplace/git.ts` (V1 `execFile("git")` wrapper -- replaced by isomorphic-git)
- `features/initial:extensions/pi-plugins/presentation/reload-hint.ts` (V1's inlined `Run /reload to ${verb}` literal -- extraction source for `RELOAD_HINT_PREFIX`)
...
- Current `extensions/pi-plugins.ts` (stub registering `pi_plugins_list` tool + `pi-plugins:list` command)
