...
- `extensions/pi-plugins/orchestrators/plugin/{install,update,uninstall,shared}.ts` -- lifecycle precedents and pitfalls.
- `extensions/pi-plugins/bridges/{skills,commands,agents,mcp}/stage.ts` -- current prepare/commit limitations.
- `extensions/pi-plugins/transaction/with-state-guard.ts` -- existing lock semantics.
