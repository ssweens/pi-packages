...
- `extensions/pi-plugins/transaction/with-state-guard.ts` - Exports `withLockedStateTransaction` using the same lock acquisition semantics as `withStateGuard` while requiring explicit `tx.save()`.
...
- `extensions/pi-plugins/transaction/with-state-guard.ts` contains `export async function withLockedStateTransaction`.
