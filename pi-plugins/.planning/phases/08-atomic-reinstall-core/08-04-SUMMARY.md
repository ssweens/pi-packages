...
key-files:
  created:
    - extensions/pi-plugins/orchestrators/plugin/reinstall.ts
...
- `extensions/pi-plugins/orchestrators/plugin/reinstall.ts` - Implements `reinstallPlugin`, installed-only preflight, cached-manifest resolution, rollback-protected bridge replacement, explicit state save, warnings, reload hints, soft-dependency notices, and data cleanup.
- `extensions/pi-plugins/orchestrators/plugin/index.ts` - Exports the reinstall orchestrator API from the plugin barrel.
- `extensions/pi-plugins/orchestrators/index.ts` - Re-exports reinstall APIs from the top-level orchestrator barrel for Phase 9 edge wiring.
- `extensions/pi-plugins/orchestrators/types.ts` - Adds shared reinstall partition and outcome types.
...
- `extensions/pi-plugins/orchestrators/plugin/reinstall.ts` exports `reinstallPlugin` and does not call `resolvePluginVersion`.
