...
- `extensions/pi-plugins/bridges/agents/types.ts` - Adds `ReplacePreparedAgentsOptions` and opaque `AgentsReplacement` handle types.
- `extensions/pi-plugins/bridges/agents/stage.ts` - Adds replace/rollback/finalize helpers with force semantics, file backup, and agents-index restore.
- `extensions/pi-plugins/bridges/agents/index.ts` - Exports new agents replacement helpers and types.
- `tests/bridges/agents/stage.test.ts` - Adds rollback/default-block/force/noop tests for agent replacement.
- `extensions/pi-plugins/bridges/mcp/types.ts` - Adds opaque `McpReplacement` handle types.
- `extensions/pi-plugins/bridges/mcp/stage.ts` - Adds replace/rollback/finalize helpers that snapshot and restore scoped `mcp.json`.
- `extensions/pi-plugins/bridges/mcp/index.ts` - Exports new MCP replacement helpers and type.
