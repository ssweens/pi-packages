...
- **D-09:** After successful reinstall, delete `<scopeRoot>/pi-plugins/data/<marketplace>/<plugin>/` and leave it absent. Do not recreate an empty data directory as part of reinstall.
...
- `extensions/pi-plugins/orchestrators/plugin/install.ts` - Cached-manifest/no-network install pattern; not suitable for reinstall composition because it rejects installed records.
- `extensions/pi-plugins/orchestrators/plugin/update.ts` - Target enumeration and staging reference; not suitable as atomicity model because state is saved before physical replacement.
- `extensions/pi-plugins/orchestrators/plugin/uninstall.ts` - Data-dir cleanup-after-state-commit warning pattern; not suitable for reinstall composition because it removes resources/state first.
- `extensions/pi-plugins/orchestrators/plugin/shared.ts` - Cross-plugin generated-name guard and version helper; reinstall must self-exempt current plugin and avoid recomputing version.
- `extensions/pi-plugins/transaction/with-state-guard.ts` - Existing lock + auto-save guard; Phase 8 extends with manual-save transaction capability.
- `extensions/pi-plugins/persistence/state-io.ts` - `loadState` / `saveState` and atomic state write behavior.
- `extensions/pi-plugins/persistence/locations.ts` - Scoped path bundle, `.state-lock`, plugin data dir, staging roots, cache paths.
- `extensions/pi-plugins/bridges/{skills,commands,agents,mcp}/stage.ts` - Current prepare/commit/abort behavior; planning must audit and add backup-capable replacement APIs.
- `extensions/pi-plugins/shared/markers.ts` - Stable marker constants; use existing manual-recovery marker discipline rather than adding a reinstall marker by default.
- `extensions/pi-plugins/shared/errors.ts` - Error classes and leak/manual-recovery composition surfaces.
- `extensions/pi-plugins/shared/notify.ts` - All user-visible warnings/errors must flow through notify wrappers.
...
- `transaction/with-state-guard.ts` already acquires `proper-lockfile` on `<scopeRoot>/pi-plugins/.state-lock` before `loadState` and releases after `saveState`. Phase 8 should factor or add a helper that exposes the same lock while giving reinstall explicit `loadState`/`saveState`/rollback control.
...
- New core file expected: `extensions/pi-plugins/orchestrators/plugin/reinstall.ts`.
- New transaction helper expected in or near `extensions/pi-plugins/transaction/with-state-guard.ts`.
