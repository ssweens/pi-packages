...
dependency_graph:
  requires:
    - extensions/pi-plugins/shared/path-safety.ts (PathContainmentError, assertPathInside)
    - extensions/pi-plugins/shared/errors.ts (errorMessage)
    - extensions/pi-plugins/persistence/locations.ts (Phase 2 ScopedLocations base)
    - extensions/pi-plugins/domain/name.ts (Phase 2 assertSafeName + generators)
  provides:
...
key-files:
  created:
    - extensions/pi-plugins/shared/vars.ts
    - extensions/pi-plugins/shared/errors-bridges.ts
  ...
  modified:
    - extensions/pi-plugins/domain/name.ts (assertSafeName label arg, B-02)
    - extensions/pi-plugins/persistence/locations.ts (5 new bridge-target fields)
...
**`extensions/pi-plugins/shared/vars.ts`** (new)
...
**`extensions/pi-plugins/shared/errors-bridges.ts`** (new)

Four typed error subclasses:

- `AgentForeignContentError` -- extends `PathContainmentError` (D-17). Carries `targetPath` and `reason`. AG-5 refusal when target file basename doesn't start with `pi-plugins-` or body lacks the marker.
...
**`extensions/pi-plugins/shared/fs-utils.ts`** (new)
...
**`extensions/pi-plugins/persistence/locations.ts`** (extended)
...
**`extensions/pi-plugins/domain/name.ts`** (extended)
...
The marker substring `generated by pi-plugins` appears in `wrong-basename.md` and `legit-with-marker.md` but is intentionally absent from `no-marker.md` (verified by grep at commit time per the plan's done-criteria).
