...
dependency_graph:
  requires:
    - extensions/pi-plugins/persistence/locations.ts (ScopedLocations.mcpJsonPath)
    - extensions/pi-plugins/shared/atomic-json.ts (atomicWriteJson)
    - extensions/pi-plugins/shared/errors-bridges.ts (McpServerCollisionError)
    - extensions/pi-plugins/shared/errors.ts (errorMessage)
    - extensions/pi-plugins/domain/name.ts (assertSafeName)
  provides:
    - bridges/mcp/types.ts (discriminated PreparedMcpStaging + StagedMcpRecord)
    - bridges/mcp/marker.ts (PI_PLUGINS_MARKER_KEY user contract)
...
tech-stack:
  ...
  patterns:
    ...
    - "Per-server _piPlugins marker is byte-for-byte V1-compatible user contract"
key-files:
  created:
    - extensions/pi-plugins/bridges/mcp/types.ts
    - extensions/pi-plugins/bridges/mcp/marker.ts
    - extensions/pi-plugins/bridges/mcp/parse.ts
    - extensions/pi-plugins/bridges/mcp/collision-slots.ts
    - extensions/pi-plugins/bridges/mcp/stage.ts
    - extensions/pi-plugins/bridges/mcp/unstage.ts
    - extensions/pi-plugins/bridges/mcp/index.ts
...
Wave 2 / parallel-bridge pass for the MCP component type. Lands the seven `bridges/mcp/*.ts` modules (six source + one barrel) and five unit-test files (51 tests total) implementing the prepare/commit/abort/unstage triplet for `mcp.json` server entries with `_piPlugins` markers.
...
- `PI_PLUGINS_MARKER_KEY = "_piPlugins"` -- user contract per MC-5; existing V1-installed `mcp.json` documents must remain readable by the successor.
...
- Marker primitives: `PI_PLUGINS_MARKER_KEY`, `buildMarker`, `readMarker`, `isOwnedBy`, type `ClaudeMarketplaceMarker`.
...
- **Files modified:** `extensions/pi-plugins/bridges/mcp/stage.ts`, `extensions/pi-plugins/bridges/mcp/unstage.ts`.
- **Commit:** `f73512b`.
...
- **Files modified:** `tests/bridges/mcp/unstage.test.ts`, `extensions/pi-plugins/bridges/mcp/index.ts`.
- **Commit:** `f73512b`.
