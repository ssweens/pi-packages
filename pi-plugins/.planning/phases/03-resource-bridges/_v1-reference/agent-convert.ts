...
export interface DiscoveredAgent {
  ...
  sourceName: string;
  /** Generated pi-subagent name -- pi-plugins-<plugin>-<stripped> */
 ...
 *  Format: `pi-plugins-<plugin>-<suffix>` where `<suffix>` strips
...
export function generateAgentName(pluginName: string, sourceAgentName: string): string {
  ...
  const suffix = sourceAgentName.startsWith(pluginName + "-")
    ...
    : sourceAgentName;
  const generatedName = `pi-plugins-${pluginName}-${suffix}`;
