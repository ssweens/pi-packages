...
key-files:
  created:
    - extensions/pi-plugins/persistence/agents-index-schema.ts
    - extensions/pi-plugins/persistence/agents-index-io.ts
...
- `extensions/pi-plugins/persistence/agents-index-schema.ts` -- TypeBox 1.x schema definitions and JIT-compiled validators. Default `Type` import + `Type.Static<typeof X>` shape (matches Phase 2 `state-io.ts`).
- `extensions/pi-plugins/persistence/agents-index-io.ts` -- `loadAgentsIndex` and `saveAgentsIndex` plus the `LoadedAgentsIndex` interface; per-row error formatting helper.
...
- **Files modified:** `extensions/pi-plugins/persistence/agents-index-io.ts` (helper inside the module instead of consuming the missing brand field).
...
- Created files exist:
  - `extensions/pi-plugins/persistence/agents-index-schema.ts` -- FOUND
  - `extensions/pi-plugins/persistence/agents-index-io.ts` -- FOUND
