...
- **`[P2]`** = depends on Phase 2 output (already shipped at `extensions/pi-plugins/`).
...
**Closest analog:** `extensions/pi-plugins/resource/stage.ts` (V1 `discoverPluginResources` skills branch, lines 46-72)
...
```typescript
export const GENERATED_AGENT_MARKER = "generated by pi-plugins";
```
...
**Role:** Utility -- AG-5 two-part marker check predicate (basename starts with `pi-plugins-` AND body contains `generated by pi-plugins`).
...
```typescript
const GENERATED_AGENT_PREFIX = "pi-plugins-";
...
**Role:** Bridge primitive -- prepare/commit/abort for MCP servers via in-memory JSON merge with `_piPlugins` markers.
...
export async function prepareStageMcpServers(input: StageMcpInput): Promise<PreparedMcpStaging> {
  ...
  for (const [name, entry] of Object.entries(servers)) {
    stamped[name] = { ...(entry as Record<string, unknown>), [PI_PLUGINS_MARKER_KEY]: marker };
  }
...
**Role:** Utility -- `_piPlugins` marker shape and ownership predicate.
**Closest analog:** V1 `mcp/marker.ts` (41 lines).
**Pattern carry-forward `[V1]`:** Carry verbatim. `PI_PLUGINS_MARKER_KEY = "_piPlugins"`, `readMarker`, `buildMarker`, `isOwnedBy`.
...
export interface ClaudeVars {
  ...
  pluginData: string;   // ${CLAUDE_PLUGIN_DATA} -- <scopeRoot>/pi-plugins/data/<mp>/<plugin>/
}
...
- `AgentForeignContentError extends PathContainmentError` -- AG-5 refusal to overwrite non-`pi-plugins-` agent file or file missing the marker. Carries `targetPath`, `reason`.
...
async function tmpExtensionRoot(): Promise<{ root: string; cleanup: () => Promise<void> }> {
  ...
  const root = path.join(dir, "pi-plugins");
...
tests/fixtures/plugins/
  ...
  foreign-agent/                     # AG-5 foreign-content refusal corpus
    ...
    agents/wrong-basename.md         # body has marker but basename doesn't start with pi-plugins-
```
...
### Atomic JSON write
**Source:** `extensions/pi-plugins/shared/atomic-json.ts` (Phase 2 carry-forward of Phase 1 D-03).
...
### Path containment
**Source:** `extensions/pi-plugins/shared/path-safety.ts::assertPathInside` (Phase 1 D-15 chokepoint).
...
**Verification:** `tests/architecture/markers-snapshot.test.ts` (existing) extends to assert `bridges/agents/frontmatter.ts::emitGeneratedAgentFile` output contains the verbatim `generated by pi-plugins` string.
...
**Analog search scope:**
- V1 source under `git show features/initial:extensions/pi-plugins/{agent,mcp,resource}/` (8 files, 2089 lines)
- Phase 2 outputs under `extensions/pi-plugins/{shared,domain,persistence}/` (verified currently shipped)
