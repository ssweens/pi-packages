...
- Scope model remains exactly `Scope = "user" | "project"` from `extensions/pi-plugins/shared/types.ts`.

## Summary

Phase 10 should create a small pure foundation under `extensions/pi-plugins/orchestrators/import/` rather than adding edge routing or mutating orchestrators. This keeps the Phase 11 command handler simple: it can call one import-plan builder, then delegate missing marketplace adds and plugin installs to existing Phase 4/5 orchestration.
...
```text
extensions/pi-plugins/orchestrators/import/
