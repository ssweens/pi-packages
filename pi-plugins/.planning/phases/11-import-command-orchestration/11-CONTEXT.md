...
- `docs/prd/pi-plugins-prd.md` §6.2 - Exactly two Pi scopes, `user` and `project`; no Claude `local` scope.
- `docs/prd/pi-plugins-prd.md` §6.5 - Completion surface and command surface conventions.
- `docs/prd/pi-plugins-prd.md` §6.6 - Argument parsing and `--scope` validation requirements.
- `docs/prd/pi-plugins-prd.md` §6.8 - Reload hint and soft-dependency warning contracts.
- `docs/prd/pi-plugins-prd.md` §6.9 - State persistence and concurrency constraints.
- `docs/prd/pi-plugins-prd.md` §6.12 - Error surfaces, severity discipline, and `ctx.ui.notify` output-channel requirement.
- `docs/prd/pi-plugins-prd.md` §10 - NFRs for atomicity, reload-only recovery, retry safety, network policy, containment, quality bar, and no telemetry.
...
- `extensions/pi-plugins/orchestrators/import/marketplaces.ts`: `buildClaudeImportPlan` already returns scoped marketplace ensure actions, plugin install actions, skipped plugins, and diagnostics.
- `extensions/pi-plugins/orchestrators/import/types.ts`: Existing import-plan and diagnostic types should be extended or wrapped rather than replaced.
- `extensions/pi-plugins/orchestrators/marketplace/add.ts`: `addMarketplace` owns source parsing, state locking, atomic clone/path recording, duplicate checks, and success notification. Phase 11 should delegate marketplace mutation here.
- `extensions/pi-plugins/orchestrators/plugin/install.ts`: `installPlugin` owns installability resolution, staging, rollback, soft-dependency warnings, resource-change notifications, and reload hints. Phase 11 should add a notification-control seam if needed rather than reimplementing this flow.
- `extensions/pi-plugins/edge/args.ts`: `parseArgs` already supports position-independent `--scope user|project`; import should reuse this behavior.
- `extensions/pi-plugins/edge/router.ts`: Top-level routing and usage strings need to include `import` consistently with existing commands.
