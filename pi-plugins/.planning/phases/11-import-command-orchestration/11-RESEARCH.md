...
- `extensions/pi-plugins/orchestrators/import/settings.ts` exposes `loadMergedClaudeSettingsForScope(scope, options)` and keeps settings discovery pure.
- `extensions/pi-plugins/orchestrators/import/refs.ts` exposes strict non-throwing `plugin@marketplace` parsing and exact-true extraction.
- `extensions/pi-plugins/orchestrators/import/marketplaces.ts` exposes `buildClaudeImportPlan(inputs)` returning per-scope `marketplacesToEnsure`, `pluginsToInstall`, `skippedPlugins`, and diagnostics.
...
- `extensions/pi-plugins/orchestrators/marketplace/add.ts` owns source parsing, GitHub/path add behavior, state locking, atomic clone/path recording, cache invalidation, and success notification.
...
- `extensions/pi-plugins/orchestrators/plugin/install.ts` owns cached-manifest resolution, installability errors, staging/rollback, state commit, cache invalidation, soft-dependency warnings, and reload hints.
...
- `extensions/pi-plugins/edge/args.ts` already parses position-independent `--scope user|project` and leaves omitted scope as `undefined`.
- `extensions/pi-plugins/edge/router.ts` needs a new top-level `import` handler, usage string entry, and dispatch branch.
- `extensions/pi-plugins/edge/completions/provider.ts` needs `import` in top-level completions. Existing flag completion already offers `--scope`; no plugin-ref completion should be added for import.
- `extensions/pi-plugins/edge/register.ts` needs to wire a new import handler. The handler needs both `pi` (for install soft-dep probes) and `deps.gitOps` (to pass to marketplace add).

## Recommended Architecture

1. Add a new orchestrator module, likely `extensions/pi-plugins/orchestrators/import/execute.ts`, with an entrypoint such as `importClaudeSettings(opts)`.
