...
- `extensions/pi-plugins/orchestrators/plugin/reinstall.ts` - Phase 8 single-plugin core, current notification behavior, `force?: boolean`, and outcome fields available for aggregation.
- `extensions/pi-plugins/orchestrators/plugin/update.ts` - Closest template for target union, scope resolution, batch enumeration, deterministic partition rendering, reload hints, and soft-dependency aggregation.
- `extensions/pi-plugins/orchestrators/types.ts` - `ReinstallPluginOutcome` fields available to Phase 9.
- `extensions/pi-plugins/orchestrators/marketplace/shared.ts` - `renderPartition` helper and deterministic partition formatting precedent.
- `extensions/pi-plugins/edge/router.ts` - Top-level usage block, handler interface, and dispatch switch that need `reinstall`.
- `extensions/pi-plugins/edge/register.ts` - Command registration handler map and completion provider wiring.
- `extensions/pi-plugins/edge/handlers/plugin/update.ts` - Thin handler template for bare, `@marketplace`, and `plugin@marketplace` forms.
- `extensions/pi-plugins/edge/args.ts` - Current `--scope` parsing and flag/positional handling; Phase 9 must add safe `--force` handling.
- `extensions/pi-plugins/edge/args-schema.ts` - Argument validation schema behavior for lifecycle handlers.
- `extensions/pi-plugins/edge/completions/provider.ts` - Top-level and plugin-ref completion branches that need `reinstall`.
- `extensions/pi-plugins/edge/completions/data.ts` - Completion data mode filtering; `reinstall` should be installed-only like update/uninstall.
- `extensions/pi-plugins/presentation/reload-hint.ts` - Canonical reload-hint composer.
- `extensions/pi-plugins/presentation/soft-dep.ts` - Existing soft-dependency warning presentation.
- `extensions/pi-plugins/platform/pi-api.ts` - Soft-dependency probing helpers and Pi API boundary.
...
- Add or extend `extensions/pi-plugins/orchestrators/plugin/reinstall.ts` with a bulk target union and `reinstallPlugins`-style entrypoint.
- Export the bulk reinstall entrypoint from `extensions/pi-plugins/orchestrators/plugin/index.ts` and any relevant root orchestrator barrel.
- Add `extensions/pi-plugins/edge/handlers/plugin/reinstall.ts` and wire it through `edge/router.ts` and `edge/register.ts`.
