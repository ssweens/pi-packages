...
- `persistence/locations.ts` extension -- new helper `pluginDataDir(loc, marketplace, plugin)` returning `<scopeRoot>/pi-plugins/data/<marketplace>/<plugin>/`
...
- `docs/prd/pi-plugins-prd.md` §5.2.1 -- PI-1..15 install rules; PI-6 cross-bridge conflict guard, PI-9 staging order, PI-14 PathContainmentError exclusion, PI-15 concurrent install
- `docs/prd/pi-plugins-prd.md` §5.2.2 -- PU-1..8 uninstall (PU-1 order, PU-2 data dir AFTER state commit, PU-5 silent converge, PU-7 foreign content, PU-8 reload hint)
- `docs/prd/pi-plugins-prd.md` §5.2.3 -- PUP-1..9 update (PUP-6 three-phase + recovery hint literal text, PUP-7 phase-3 abort, PUP-9 cascade vs direct)
- `docs/prd/pi-plugins-prd.md` §5.3.1 -- PL-1..7 top-level list (PL-4 icon table, PL-5 upgradable comparator, PL-6 manifest soft-fail, PL-7 [autoupdate] tag)
- `docs/prd/pi-plugins-prd.md` §6.4 -- PR-1..6 resolver; PR-4 is SUPERSEDED by Phase 5 D-07 (supplement-not-replace)
- `docs/prd/pi-plugins-prd.md` §6.5 -- RN-1..6 resource naming + `assertSafeName`; RN-3 cross-plugin conflict guard timing ("BEFORE any disk write"); RN-4 cross-marketplace agent ownership (bridge-enforced)
- `docs/prd/pi-plugins-prd.md` §6.8 -- RH-1..5 reload hint format + soft-dep probing; PU-8 install/uninstall reload hint and PUP-8 update reload hint
- `docs/prd/pi-plugins-prd.md` §6.9 -- ST-1..9 state persistence; ST-7 `withStateGuard` mandate; ST-8 concurrent install/uninstall; ST-9 concurrent update
- `docs/prd/pi-plugins-prd.md` §6.10 -- PS-1..5 path safety; PS-4 containment violations during rollback propagate (matches PI-14 + D-02 enforcement)
- `docs/prd/pi-plugins-prd.md` §6.11 -- AS-1..9 atomic staging; AS-2 install order, AS-3 update three-phase, AS-4 rollback-partial format, AS-6 post-commit leak warning severity, AS-7 orphan agent index entries
- `docs/prd/pi-plugins-prd.md` §6.12 -- ES-1..5 error surfaces; ES-5 stable user-contract markers
- `docs/prd/pi-plugins-prd.md` §10 -- NFR-2 no Pi restart, NFR-3 idempotent/fail-clean
- `docs/prd/pi-plugins-prd.md` §11 -- V1 deferrals (COMP-01 was here; Phase 5 D-07 brings it in-scope as a behavior-corrected-vs-V1 ship)
...
- `extensions/pi-plugins/shared/notify.ts` -- `notifySuccess`, `notifyWarning`, `notifyError(ctx, msg, cause?)`. Every Phase 5 user-visible message routes through these.
- `extensions/pi-plugins/shared/markers.ts` -- ES-5 strings + `RELOAD_HINT_PREFIX`. Phase 5 ADDS `RECOVERY_PLUGIN_REINSTALL_PREFIX` here.
- `extensions/pi-plugins/shared/path-safety.ts` -- `assertPathInside` + `SymlinkRefusedError`; called on every name-derived path (data dir, staging dir, bridge target paths).
- `extensions/pi-plugins/shared/atomic-json.ts` -- `atomicWriteJson` consumed indirectly via state-io and agents-index-io.
- `extensions/pi-plugins/shared/errors.ts` -- `PathContainmentError`, `appendLeakToError`, `appendLeaks`, `errorMessage`. Phase 5 ADDS `CrossPluginConflictError`, `ConcurrentInstallError`, `ConcurrentUninstallError`, `PluginUpdatePhase3Error`.
- `extensions/pi-plugins/shared/fs-utils.ts` -- `cleanupStaging`, `pathExists`. Phase 5 install + update prepare-rollback paths consume both.
...
- `extensions/pi-plugins/domain/resolver.ts` -- `resolveStrict`, `resolveLoose`, `requireInstallable`; Phase 5 D-07 CHANGES `ComponentPathsSchema` to readonly-string-array; supersedes PR-4
- `extensions/pi-plugins/domain/manifest.ts` -- `MARKETPLACE_VALIDATOR`, `PLUGIN_MANIFEST_VALIDATOR`; install reads cached manifest only (PI-2)
- `extensions/pi-plugins/domain/name.ts` -- `assertSafeName` + skill/command/agent generators; consumed by D-05 PI-6 guard for input computation
- `extensions/pi-plugins/domain/version.ts` -- `computeHashVersion` + `HASH_WALK_SKIP`; install's PI-7 version resolution and update's PUP-3 unchanged-detection both use this
- `extensions/pi-plugins/persistence/locations.ts` -- `ScopedLocations` brand + `locationsFor(scope, cwd)`; Phase 5 ADDS `pluginDataDir(loc, marketplace, plugin)` helper (D-08)
- `extensions/pi-plugins/persistence/state-io.ts` -- `loadState`, `saveState`, `STATE_VALIDATOR`; install/uninstall/update mutate state via `withStateGuard` closures
- `extensions/pi-plugins/persistence/migrate.ts` -- `migrateLegacyMarketplaceRecords`; ST-4/ST-5 + PU-6 legacy-state-load handling
- `extensions/pi-plugins/transaction/phase-ledger.ts` -- `runPhases<C>`, `Phase<C>`; install.ts consumes
- `extensions/pi-plugins/transaction/rollback.ts` -- `formatRollbackError`; Phase 5 D-02 EXTENDS with PI-14 bypass logic
- `extensions/pi-plugins/transaction/with-state-guard.ts` -- `withStateGuard`; every Phase 5 mutating orchestrator wraps in this
...
- `extensions/pi-plugins/bridges/skills/index.ts` -- `prepareStageSkills`, `commitPreparedSkills`, `abortPreparedSkills`, `unstagePluginSkills`, `assertNoSkillCollisions`, `discoverPluginSkills`. Phase 5 D-07 CHANGES `discoverPluginSkills` to iterate array of component paths.
- `extensions/pi-plugins/bridges/commands/index.ts` -- same family. Phase 5 D-07 CHANGES `discoverPluginCommands` to iterate array.
- `extensions/pi-plugins/bridges/agents/index.ts` -- `prepareStagePluginAgents`, `commitPreparedAgents`, `abortPreparedAgents`, `unstagePluginAgents`, `findOwnershipConflicts`, `discoverPluginAgents`. RN-4 cross-marketplace ownership stays bridge-enforced (D-05 corollary). Phase 5 D-07 CHANGES `discoverPluginAgents` to iterate array.
- `extensions/pi-plugins/bridges/mcp/index.ts` -- `prepareStageMcpServers`, `commitPreparedMcp`, `abortPreparedMcp`, `unstageMcpServers`, `MCP_COLLISION_SLOTS`. MC-4 same-kind collision stays bridge-enforced (excluded from PI-6 per D-05).
- `extensions/pi-plugins/persistence/agents-index-io.ts` -- agents-index round-trip; uninstall's PU-7 foreign-content refusal surfaces through the index
...
- `extensions/pi-plugins/orchestrators/marketplace/shared.ts` -- `cascadeUnstagePlugin(plugin, marketplace, locations)`, `GitOps` interface, `DEFAULT_GIT_OPS`, `resolveScopeFromState`, `formatErrorWithCauses`. Phase 5's `uninstall.ts` imports `cascadeUnstagePlugin` directly; `update.ts` imports `GitOps` + `DEFAULT_GIT_OPS` for the PUP-2 syncClone.
- `extensions/pi-plugins/orchestrators/types.ts` -- `PluginUpdateFn`, `PluginUpdateOutcome`, `PluginUpdatePartition`. Phase 5 D-09 corollary exports the real `updateSinglePlugin: PluginUpdateFn` from `orchestrators/plugin/update.ts`.
- `extensions/pi-plugins/presentation/reload-hint.ts` -- `composeReloadHint(changedNames, verb)`. Install/uninstall/update emit PU-8/PUP-8 hints via this.
- `extensions/pi-plugins/presentation/soft-dep.ts` -- `softDepStatus(ctx)` returning `{ subagentsLoaded, mcpAdapterLoaded }`; PI-11/PI-12 (install) and RH-5 (update phase-3b) consume.
- `extensions/pi-plugins/presentation/marketplace-list.ts` -- PRESENTATION LAYER pattern that Phase 5 D-06 mirrors with `presentation/plugin-list.ts`.
- `extensions/pi-plugins/orchestrators/marketplace/list.ts` -- ORCHESTRATOR LAYER pattern that Phase 5 D-06 mirrors with `orchestrators/plugin/list.ts`.
...
- `git show features/initial:extensions/pi-plugins/plugin/install.ts` -- V1 install orchestrator; 4-phase ordering pattern. NOTE: V1 has the COMP-01 BUG (component-path replacement rather than supplement) -- Phase 5 D-07 corrects.
- `git show features/initial:extensions/pi-plugins/plugin/uninstall.ts` -- V1 uninstall; PU-1 ordering reference
- `git show features/initial:extensions/pi-plugins/plugin/update.ts` -- V1 update three-phase; PUP-6 recovery hint phrasing reference
- `git show features/initial:extensions/pi-plugins/plugin/list.ts` -- V1 list; PL-4 icon table + column-66 truncation reference
- `git show features/initial:extensions/pi-plugins/resolver/*` -- V1 resolver; PR-4 short-circuit behavior (the bug Phase 5 D-07 corrects)
...
- **`extensions/pi-plugins/shared/markers.ts`** -- Phase 5 ADDS `RECOVERY_PLUGIN_REINSTALL_PREFIX = "plugin-uninstall + plugin-install for"` here. Existing 5 ES-5 prefix constants stay unchanged; the snapshot test (`tests/architecture/markers-snapshot.test.ts`) gains one new prefix-equivalence case asserting against PRD §5.2.3 PUP-6.
- **`extensions/pi-plugins/shared/errors.ts`** -- Phase 5 ADDS four new error classes:
...
- **`extensions/pi-plugins/transaction/rollback.ts`** -- Phase 5 D-02 EXTENDS `formatRollbackError(result, originalError)` with an `instanceof PathContainmentError` short-circuit (covers `SymlinkRefusedError`). On match: return originalError verbatim, suppress the `(rollback partial: …)` summary. Import: `import { PathContainmentError } from "../shared/errors.ts"`.
- **`extensions/pi-plugins/transaction/phase-ledger.ts`** -- `runPhases<C>` + `Phase<C>`. Install.ts is the first production consumer. The literal-array discipline (Phase 2 D-01) is binding -- `const phases: readonly Phase<InstallCtx>[] = [...]` at the install.ts callsite.
- **`extensions/pi-plugins/transaction/with-state-guard.ts`** -- Install, uninstall, update each wrap their flow in one. Update's swap phase is the closure body (PUP-6 phase-2); install's closure body is the entire `runPhases` call (Phase 2 D-02 verbatim composition); uninstall's closure body is the cascade + state-record-removal.
- **`extensions/pi-plugins/persistence/locations.ts`** -- Phase 5 ADDS `pluginDataDir(loc: ScopedLocations, marketplace: string, plugin: string): string` returning `<scopeRoot>/pi-plugins/data/<marketplace>/<plugin>/`. Goes through `assertPathInside` against the scope root. Used by install (eager mkdir post-state-commit) and uninstall (rm-rf post-state-commit per PU-2/PU-4) and by the PI-10 `${CLAUDE_PLUGIN_DATA}` substitution input (bridges already accept the resolved path from the orchestrator).
- **`extensions/pi-plugins/domain/resolver.ts`** -- Phase 5 D-07 CHANGES `ComponentPathsSchema` to readonly-string-array-per-kind; strict and loose resolver Step 7 logic changes accordingly. Supersedes PR-4.
- **`extensions/pi-plugins/domain/name.ts`** -- `assertSafeName` + the three generators; D-05's PI-6 guard input computation reuses these.
- **`extensions/pi-plugins/domain/version.ts`** -- `computeHashVersion` + `HASH_WALK_SKIP`. Install's PI-7 fallback uses this; update's PUP-3 uses string equality on either `manifest.version` or `hash-<12hex>` (precedent: Phase 2 hash-stability snapshot test).
- **`extensions/pi-plugins/bridges/{skills,commands,agents,mcp}/index.ts`** -- `prepare*`, `commit*`, `abort*`, `unstage*` primitives. Install ledger phases call these directly; update three-phase calls them across the three explicit steps; uninstall reuses `cascadeUnstagePlugin` from Phase 4 (which itself calls these).
- **`extensions/pi-plugins/orchestrators/marketplace/shared.ts`** -- `cascadeUnstagePlugin`, `GitOps`, `DEFAULT_GIT_OPS`, `resolveScopeFromState`, `formatErrorWithCauses`. Phase 5 uninstall imports `cascadeUnstagePlugin` directly; update imports `GitOps` + `DEFAULT_GIT_OPS` for PUP-2 syncClone.
- **`extensions/pi-plugins/orchestrators/types.ts`** -- `PluginUpdateFn`, `PluginUpdateOutcome`, `PluginUpdatePartition`. Phase 5 ships the implementation matching `PluginUpdateFn`; threading `stagedAgents`/`stagedMcpServers` (WR-04 fields already in place) into RH-5 soft-dep warning composition.
- **`extensions/pi-plugins/presentation/{reload-hint,soft-dep,marketplace-list}.ts`** -- Phase 5 install/uninstall/update consume `composeReloadHint` + `softDepStatus`. `marketplace-list.ts` is the orchestrator+presentation split PRECEDENT that Phase 5 D-06 mirrors.
