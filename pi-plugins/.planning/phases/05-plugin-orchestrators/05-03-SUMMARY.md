...
- **`bridges/agents/discover.ts`** -- Signature flip: `agentsDir: string` -> `agentsDirs: readonly string[]`. The frontmatter parse + `sourceHash` over raw bytes path is unchanged. Dedup by `generatedAgentName` (`pi-plugins-<plugin>-<agent>`).
...
```
pluginDataDir("ok", "p/sub") -> /tmp/proj/.pi/pi-plugins/data/ok/p/sub
pluginDataDir("ok", "p\\sub") -> /tmp/proj/.pi/pi-plugins/data/ok/p\sub
pluginDataDir("a", "b/../escape") -> /tmp/proj/.pi/pi-plugins/data/a/escape
```
...
- **Files modified:** `extensions/pi-plugins/persistence/locations.ts`, `tests/persistence/locations.test.ts`
- **Commit:** `8fdd32c`
...
- `[FOUND]` `extensions/pi-plugins/domain/resolver.ts`
- `[FOUND]` `extensions/pi-plugins/persistence/locations.ts`
- `[FOUND]` `extensions/pi-plugins/bridges/skills/discover.ts`
- `[FOUND]` `extensions/pi-plugins/bridges/commands/discover.ts`
- `[FOUND]` `extensions/pi-plugins/bridges/agents/discover.ts`
