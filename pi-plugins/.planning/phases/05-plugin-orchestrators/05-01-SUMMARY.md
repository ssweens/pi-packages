...
key-files:
  ...
  modified:
    - extensions/pi-plugins/shared/markers.ts (added RECOVERY_PLUGIN_REINSTALL_PREFIX)
    - extensions/pi-plugins/shared/errors.ts (added 4 error classes + Phase3Failure interface)
...
- `extensions/pi-plugins/shared/markers.ts` -- Added `RECOVERY_PLUGIN_REINSTALL_PREFIX` constant with JSDoc noting Phase 5 extension beyond ES-5.
- `extensions/pi-plugins/shared/errors.ts` -- Added 4 error classes (`CrossPluginConflictError`, `ConcurrentInstallError`, `ConcurrentUninstallError`, `PluginUpdatePhase3Error`) and `Phase3Failure` interface; appended after the existing `MarketplaceUpdateError` block.
...
- `extensions/pi-plugins/shared/markers.ts` -- FOUND, contains `RECOVERY_PLUGIN_REINSTALL_PREFIX = "plugin-uninstall + plugin-install for"`.
- `extensions/pi-plugins/shared/errors.ts` -- FOUND, exports `CrossPluginConflictError`, `ConcurrentInstallError`, `ConcurrentUninstallError`, `PluginUpdatePhase3Error`, `Phase3Failure`.
