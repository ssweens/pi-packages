...
- **Created:** `extensions/pi-plugins/orchestrators/plugin/install.ts` (644 lines) -- the `installPlugin` entrypoint, `InstallPluginOptions` and local `InstallCtx` types, `loadCachedMarketplaceManifest` / `resolveInstallVersion` / `pickAgentsSourceDir` helpers, and the 5-phase ledger composition.
