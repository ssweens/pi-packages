...
| File (path absolute under `extensions/pi-plugins/`) | New / Modified | Lines (est.) | Purpose |
|------|----------------|-----:|---------|
...
| `extensions/pi-plugins/orchestrators/index.ts` | MODIFIED | +4 lines | Barrel re-exports |
...
test("PI-2 / NFR-5: install.ts has zero gitOps surface imports", async () => {
  const src = await readFile("extensions/pi-plugins/orchestrators/plugin/install.ts", "utf8");
...
**Where verified:** Every `[VERIFIED]` claim in this document corresponds to a file path under `/Users/acolomba/src/pi-plugins/` that was read in this research session. Where `[ASSUMED]` is unavoidable (A2, A4, A8, A9), the plan-checker should confirm during planning.
...
- `/Users/acolomba/src/pi-plugins/.planning/phases/05-plugin-orchestrators/05-CONTEXT.md` (the binding contract)
- `/Users/acolomba/src/pi-plugins/.planning/phases/05-plugin-orchestrators/05-DISCUSSION-LOG.md` (alternatives audit)
- `/Users/acolomba/src/pi-plugins/.planning/REQUIREMENTS.md` (REQ-IDs)
- `/Users/acolomba/src/pi-plugins/.planning/ROADMAP.md` lines 110-160 (Phase 5 goal + 6 success criteria + Phase 4 / 5 progress)
- `/Users/acolomba/src/pi-plugins/.planning/config.json` (workflow flags)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/transaction/phase-ledger.ts` (`runPhases<C>` + `Phase<C>` + `RollbackPartial` + `RunPhasesResult`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/transaction/rollback.ts` (`formatRollbackError`, ROLLBACK_PARTIAL chokepoint -- D-02 extension target)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/transaction/with-state-guard.ts` (ST-7 wrapper; outer-guard / inner-ledger composition pattern)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/types.ts` (`PluginUpdateFn`, `PluginUpdateOutcome`, `PluginUpdatePartition` -- Phase 5 D-09 implementation surface)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/shared.ts` (`GitOps`, `DEFAULT_GIT_OPS`, `cascadeUnstagePlugin`, `resolveScopeFromState`, `formatErrorWithCauses`, `applyAutoupdateFlipInPlace`, `AgentsUnstageFailureError`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/add.ts` (atomic staging-dir-then-rename + withStateGuard + appendLeaks pattern; reference for install staging-dir leak handling)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/remove.ts` (post-state-commit cleanup + per-plugin cascade injection seam + MR-3/MR-6 aggregation -- the closest analog to uninstall.ts)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/list.ts` (orchestrator-presentation split; D-04 corollary: no withStateGuard for read-only)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/update.ts` (D-14 follow-upstream-blindly sequence; partition rendering MU-7; soft-dep WR-04 fields; cloneAdvanced CR-05; outer-guard-cascade-outside D-08 -- the closest analog to update.ts)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/orchestrators/marketplace/autoupdate.ts` (idempotent flip pattern; SC-6 enumeration)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/domain/resolver.ts` (`ComponentPathsSchema`, `resolveStrict`, `resolveLoose`, `validateComponentPath`, `requireInstallable` -- D-07 migration target)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/domain/manifest.ts` (`MARKETPLACE_VALIDATOR`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/domain/components/plugin.ts` (`PluginEntry`, `PluginManifest` -- `dependencies` is `Type.Optional(Type.Unknown())`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/domain/version.ts` (`computeHashVersion`, `HASH_WALK_SKIP`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/persistence/locations.ts` (`pluginDataDir`/`marketplaceDataDir`/`sourceCloneDir` methods on ScopedLocations; A1 verification)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/persistence/state-io.ts` (`STATE_SCHEMA`, `PLUGIN_INSTALL_RECORD_SCHEMA`, `loadState`/`saveState` -- `resources.{skills,prompts,agents,mcpServers}: string[]`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/bridges/skills/{discover,stage,unstage,types}.ts` (signature surface for D-07 + install ledger phases)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/bridges/commands/{discover,stage,types}.ts` (same)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/bridges/agents/{discover,stage,unstage,marker,index,types}.ts` (foreign-content marker; `findOwnershipConflicts`; `prepareStagePluginAgents` throws on AG-9; `commitPreparedAgents` preserves foreign-preserved entries)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/bridges/mcp/{stage,unstage,parse,index,types}.ts` (`McpServerCollisionError`, MC-4 / MC-6 / MC-7 surfaces; `loadEffectiveServerNames`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/shared/markers.ts` (5 ES-5 prefixes; D-04 extension target)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/shared/errors.ts` (existing error classes; `appendLeakToError`, `appendLeaks`, `errorMessage`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/shared/notify.ts` (sole `ctx.ui.notify` chokepoint; severity-named wrappers)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/shared/fs-utils.ts` (`cleanupStaging`, `pathExists`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/presentation/marketplace-list.ts` (D-06 precedent; icon constant; payload-driven render; `[autoupdate]` suffix shape)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/presentation/reload-hint.ts` (`reloadHint`, `appendReloadHint`, `ReloadVerb`)
- `/Users/acolomba/src/pi-plugins/extensions/pi-plugins/presentation/soft-dep.ts` (`subagentWarningIfNeeded`, `mcpAdapterWarningIfNeeded`, ExtensionAPI vs ExtensionContext gotcha)
- `/Users/acolomba/src/pi-plugins/tests/orchestrators/marketplace/list.test.ts` (test pattern: hermetic HOME, NotifyRecord ctx, ML-3 + NFR-5 source-grep, `stripComments`)
- `/Users/acolomba/src/pi-plugins/tests/orchestrators/marketplace/update.test.ts` first 100 lines (fixture pattern, makeMockGitOps, seedGithubMarketplace, withHermeticHome, makeGithubSource)
- `/Users/acolomba/src/pi-plugins/.planning/phases/04-marketplace-orchestrators/04-CONTEXT.md` (D-01..D-14; the architectural family Phase 5 mirrors)
- `/Users/acolomba/src/pi-plugins/.planning/phases/04-marketplace-orchestrators/04-RESEARCH.md` (Phase 4 research -- the methodology Phase 5 mirrors)

### Secondary (MEDIUM confidence -- referenced by metadata only, not deeply read)

- `docs/prd/pi-plugins-prd.md` §§5.2.1 / 5.2.2 / 5.2.3 / 5.3.1 / 6.4 / 6.5 / 6.8 / 6.9 / 6.10 / 6.11 / 6.12 -- PRD source of all REQ-IDs and ES-5 marker prefix strings. Read selectively during planning to verify D-04 byte-for-byte and PU-5 notification policy.
...
These are the load-bearing directives from `/Users/acolomba/src/pi-plugins/CLAUDE.md` that Phase 5 plans MUST honor verbatim:
