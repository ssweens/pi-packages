...
- `extensions/pi-plugins/persistence/locations.ts` - Adds the per-scope `.state-lock` sentinel path.
- `extensions/pi-plugins/shared/markers.ts` - Adds the Phase 7 lock-held marker prefix.
- `extensions/pi-plugins/shared/errors.ts` - Adds `StateLockHeldError` with scope and lock-path details.
- `extensions/pi-plugins/transaction/with-state-guard.ts` - Acquires and releases the cross-process lock around state mutation.
...
- **Issue:** `proper-lockfile.lock(locations.extensionRoot, ...)` requires the lock target directory to exist. A fresh scope might not have `<scopeRoot>/pi-plugins/` yet.
- **Fix:** Added `mkdir(locations.extensionRoot, { recursive: true })` before lock acquisition.
- **Files modified:** `extensions/pi-plugins/transaction/with-state-guard.ts`
...
- Found `extensions/pi-plugins/transaction/with-state-guard.ts`.
