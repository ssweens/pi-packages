...
- `extensions/pi-plugins/domain/manifest.ts` - owns the manifest read and validation seam.
- `extensions/pi-plugins/orchestrators/marketplace/add.ts` - uses the seam for cloned and local path-source marketplace manifests.
- `extensions/pi-plugins/orchestrators/marketplace/update.ts` - validates refreshed manifests through the seam.
- `extensions/pi-plugins/orchestrators/plugin/install.ts` - loads cached marketplace manifests through the seam.
- `extensions/pi-plugins/orchestrators/plugin/list.ts` - soft-fail manifest loading now delegates to the seam.
- `extensions/pi-plugins/orchestrators/plugin/update.ts` - update manifest loading now delegates to the seam.
- `extensions/pi-plugins/orchestrators/edge-deps.ts` - completion cache resolver now delegates manifest reads to the seam.
...
- **Files modified:** `extensions/pi-plugins/orchestrators/edge-deps.ts`
