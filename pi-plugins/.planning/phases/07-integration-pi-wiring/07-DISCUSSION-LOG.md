...
| Extend tests/integration/ + env-gated | Fold e2e under tests/integration/ with `PI_PLUGINS_E2E_REF` env var. Single workflow with matrix. | |
...
| Option | Description | Selected |
|--------|-------------|----------|
| Per-scope lock | Lockfile at `<scopeRoot>/pi-plugins/.state-lock`. User and project scope locks independent. | ✓ |
