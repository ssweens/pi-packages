...
- **D-11 (Disk walk over state.json):** For each scope, list subdirectories of `<scopeRoot>/pi-plugins/resources/skills/` (each subdirectory is a `<plugin>-<skill>/` with a `SKILL.md` inside; the returned path is the directory or `SKILL.md` per Pi's `resources_discover` event contract -- confirmed during research). List `*.md` files in `<scopeRoot>/pi-plugins/resources/prompts/`. Source-of-truth is disk -- if state.json is stale or a user manually deletes a file, discovery reflects reality.
...
- **D-04 ESLint rule exact pattern + exception list:** Planning picks the precise glob; default exempts `platform/pi-api.ts` itself + `tests/**/*` + `extensions/pi-plugins/index.ts` (if it imports for the `ExtensionAPI` parameter type).
...
- `docs/prd/pi-plugins-prd.md` §5.5 -- SK-1..5 skills bridge (SK-5 `resources_discover` per-scope error aggregation)
- `docs/prd/pi-plugins-prd.md` §5.6 -- CM-1..4 commands bridge (CM-4 flat `*.md` files non-recursive)
- `docs/prd/pi-plugins-prd.md` §6.8 -- RH-3, RH-4 soft-dep probing rules (consumed by `platform/pi-api.ts::softDepStatus`)
- `docs/prd/pi-plugins-prd.md` §6.9 -- ST-1..9 state persistence (ST-7 `withStateGuard`; ST-8 concurrent install/uninstall; D-06..D-09 introduce the cross-process lock that extends ST-7's semantics)
- `docs/prd/pi-plugins-prd.md` §6.12 -- ES-1..5 error surfaces (ES-2 severity ladder; ES-5 stable user-contract markers -- `STATE_LOCK_HELD_PREFIX` is a new ES-5 extension)
- `docs/prd/pi-plugins-prd.md` §10 -- NFR-2 (no Pi restart -- verified by e2e), NFR-3 (idempotent/fail-clean -- verified by concurrent-install test), NFR-8 (manifest mtime cache -- D-14 ships the seam), NFR-11 (peer-dep floor -- D-16 pins)
- `docs/prd/pi-plugins-prd.md` §11 -- V1 deferrals; PI-15 supersession recorded as a Phase 7 D-08 contract change
...
- `extensions/pi-plugins/shared/notify.ts` -- every Phase 7 user-visible message routes through `notifyError(ctx, msg, cause?)`; the lock-acquisition failure surfaces here
- `extensions/pi-plugins/shared/markers.ts` -- Phase 7 ADDS `STATE_LOCK_HELD_PREFIX` here
- `extensions/pi-plugins/shared/errors.ts` -- Phase 7 ADDS `StateLockHeldError` and `AggregateResourcesDiscoverError`
- `extensions/pi-plugins/shared/atomic-json.ts` -- `atomicWriteJson` is the durability layer under the new lock layer
...
- `extensions/pi-plugins/transaction/with-state-guard.ts` -- Phase 7 D-06 EXTENDS to acquire/release `proper-lockfile` around the existing load/save sequence
- `extensions/pi-plugins/persistence/locations.ts` -- `ScopedLocations` brand. Phase 7 ADDS a `stateLockFile(loc)` helper returning `<scopeRoot>/pi-plugins/.state-lock` (containment via `assertPathInside`)
- `extensions/pi-plugins/persistence/state-io.ts` -- `loadState`, `saveState`; Phase 7's lock layer wraps these in `withStateGuard` without changing their signatures
- `extensions/pi-plugins/domain/manifest.ts` -- the NFR-8 caching seam; Phase 7's architecture test asserts this file is the sole `marketplace.json` reader
...
- `extensions/pi-plugins/orchestrators/marketplace/shared.ts` -- `DEFAULT_GIT_OPS` import target for `index.ts`
- `extensions/pi-plugins/orchestrators/types.ts` -- `PluginUpdateFn`, `PluginUpdateOutcome`
- `extensions/pi-plugins/orchestrators/edge-deps.ts` -- `makeLocationsResolver`; Phase 6 D-04 created this; `index.ts` consumes via `registerClaudePluginCommand`
...
- `extensions/pi-plugins/orchestrators/plugin/update.ts` -- exports `updateSinglePlugin: PluginUpdateFn` for `index.ts` injection
...
- `extensions/pi-plugins/edge/register.ts` -- `registerClaudePluginCommand`, `registerClaudeMarketplaceTools`; Phase 7's `index.ts` calls both
- `extensions/pi-plugins/edge/types.ts` -- `EdgeDeps` interface; Phase 7 builds an instance
...
- `git show features/initial:extensions/pi-plugins.ts` -- V1 entrypoint; the registration shape Phase 7's `index.ts` matches (modulo Phase 6's two-helper split via `registerClaudePluginCommand` + `registerClaudeMarketplaceTools`)
...
- **`extensions/pi-plugins/index.ts`** -- the Phase 1 stub returns empty `resources_discover` arrays and a "not implemented yet" warning for the slash command. Phase 7 REPLACES the stub with a 3-call entrypoint (D-10 + D-13 + Phase 6 D-04 registration).
- **`extensions/pi-plugins/edge/register.ts`** -- already exports `registerClaudePluginCommand(pi, deps)` + `registerClaudeMarketplaceTools(pi)`. Phase 7's `index.ts` calls both. No changes to `register.ts`.
- **`extensions/pi-plugins/edge/types.ts`** -- `EdgeDeps` shape locked. Phase 7's `index.ts` builds the instance from `DEFAULT_GIT_OPS` + `updateSinglePlugin`.
- **`extensions/pi-plugins/orchestrators/marketplace/shared.ts`** -- `DEFAULT_GIT_OPS` already exported. Phase 7 imports.
- **`extensions/pi-plugins/orchestrators/plugin/update.ts`** -- `updateSinglePlugin: PluginUpdateFn` already exported. Phase 7 imports.
- **`extensions/pi-plugins/orchestrators/edge-deps.ts`** -- `makeLocationsResolver` already exists; `register.ts` already calls it. No changes for Phase 7 unless the resolver needs a tweak for `resources_discover`'s per-call cwd capture (D-13).
- **`extensions/pi-plugins/transaction/with-state-guard.ts`** -- Phase 7 D-06 EXTENDS to acquire `proper-lockfile` lock before `loadState`, release after `saveState`. Caller signatures unchanged.
- **`extensions/pi-plugins/persistence/state-io.ts`** -- `loadState`, `saveState` consumed unchanged. The lock layer wraps these.
- **`extensions/pi-plugins/persistence/locations.ts`** -- Phase 7 ADDS `stateLockFile(loc: ScopedLocations): string` returning `<scopeRoot>/pi-plugins/.state-lock` (containment via `assertPathInside`).
- **`extensions/pi-plugins/domain/manifest.ts`** -- the NFR-8 seam. No code change in Phase 7; the architecture test verifies the seam.
- **`extensions/pi-plugins/presentation/soft-dep.ts`** -- Phase 7 D-03 MOVES `softDepStatus`, `hasLoadedPiSubagents`, `hasLoadedPiMcpAdapter` to `platform/pi-api.ts`. This file becomes a one-line re-export shim so Phase 4/5 callsites stay green.
- **`extensions/pi-plugins/shared/markers.ts`** -- Phase 7 ADDS `STATE_LOCK_HELD_PREFIX = "Another pi-plugins operation is in progress for"`. Markers-snapshot test gains one case.
- **`extensions/pi-plugins/shared/errors.ts`** -- Phase 7 ADDS `StateLockHeldError extends Error` (carries the scope name) and `AggregateResourcesDiscoverError extends Error` (carries the per-scope error array via `Error.cause`).
...
- **E2E Layer A (programmatic) consumes `index.ts` directly:** No new entrypoint needed; tests `import claudeMarketplaceExtension from "../../extensions/pi-plugins/index.ts"` and instantiate it with a tiny `mockPi: ExtensionAPI` record-and-replay shim.
...
- **`tests/architecture/manifest-read-seam.test.ts`:** Walk every `.ts` file under `extensions/pi-plugins/` (excluding `domain/manifest.ts` and `tests/`). Assert no source-text occurrence of `marketplace.json` in a `readFile`-call context (regex-based check matching the precedent of `no-orchestrator-network.test.ts`).
...
- **Package.json scripts:**
  ...
  "test:e2e": "PI_PLUGINS_E2E_REF=pinned node --test \"tests/e2e/**/*.test.ts\"",
  "test:e2e:nightly": "PI_PLUGINS_E2E_REF=main node --test \"tests/e2e/**/*.test.ts\""
  ```
