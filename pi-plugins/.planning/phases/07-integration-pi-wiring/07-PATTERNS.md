...
|-------------------|------|-----------|----------------|---------------|
| `extensions/pi-plugins/index.ts` | route / entrypoint | event-driven + request-response | `extensions/pi-plugins/edge/register.ts` | role-match |
| `extensions/pi-plugins/platform/pi-api.ts` | platform wrapper | request-response | `extensions/pi-plugins/platform/git.ts` + `presentation/soft-dep.ts` | role-match |
| `extensions/pi-plugins/presentation/soft-dep.ts` | presentation shim | transform | `extensions/pi-plugins/presentation/index.ts` pattern: barrel/shim export | partial |
| `extensions/pi-plugins/orchestrators/discover.ts` | orchestrator | file-I/O transform | `extensions/pi-plugins/bridges/skills/discover.ts` + `bridges/commands/discover.ts` | data-flow-match |
| `extensions/pi-plugins/transaction/with-state-guard.ts` | transaction | CRUD + file-I/O | existing same file | exact |
| `extensions/pi-plugins/persistence/locations.ts` | persistence utility | transform / file-I/O path construction | existing same file | exact |
| `extensions/pi-plugins/shared/markers.ts` | shared constants | transform | existing same file | exact |
| `extensions/pi-plugins/shared/errors.ts` | shared errors | error propagation | existing same file | exact |
| `extensions/pi-plugins/domain/manifest.ts` | domain model / utility | file-I/O seam + validation | existing same file | role-match |
...
### `extensions/pi-plugins/index.ts` (route / entrypoint, event-driven + request-response)

**Analog:** `extensions/pi-plugins/edge/register.ts`
...
### `extensions/pi-plugins/platform/pi-api.ts` (platform wrapper, request-response)
...
### `extensions/pi-plugins/presentation/soft-dep.ts` (presentation shim, transform)
...
### `extensions/pi-plugins/orchestrators/discover.ts` (orchestrator, file-I/O transform)
...
### `extensions/pi-plugins/transaction/with-state-guard.ts` (transaction, CRUD + file-I/O)
...
### `extensions/pi-plugins/persistence/locations.ts` (persistence utility, path construction)

**Analog:** existing same file.

**Path field pattern** (lines 117-132):
```typescript
const extensionRoot = path.join(scopeRoot, "pi-plugins");
...
**Apply to Phase 7:** add a `readonly stateLockFile: string` or `stateLockFile(loc): string` helper for `<scopeRoot>/pi-plugins/.state-lock`. If helper is async, use `assertPathInside(extensionRoot, candidate, ...)`; if field is sync, follow existing hard-coded suffix rationale.

---

### `extensions/pi-plugins/shared/markers.ts` and `shared/errors.ts` (shared constants/errors)
...
**Apply to Phase 7:** add `STATE_LOCK_HELD_PREFIX = "Another pi-plugins operation is in progress for"`; add `StateLockHeldError` carrying scope/path; add `AggregateResourcesDiscoverError` carrying per-scope failures and `cause` chain.

---

### `extensions/pi-plugins/domain/manifest.ts` (domain model / file-I/O seam)
...
**Apply to Phase 7:** scan `extensions/pi-plugins/**/*.ts` excluding `domain/manifest.ts`; assert no stripped source has `readFile`/`fs.readFile` call context containing `marketplace.json`. Reuse explicit offender messages.
...
**Apply to Phase 7:** add `no-restricted-imports` for `@mariozechner/pi-coding-agent` in extension files. Exempt `extensions/pi-plugins/platform/pi-api.ts` and tests. Keep direct peer imports in tests allowed.
...
**Analog search scope:** `extensions/pi-plugins/**/*.ts`, `tests/**/*.test.ts`, `.github/workflows/*.yml`, `eslint.config.js`, `package.json`
