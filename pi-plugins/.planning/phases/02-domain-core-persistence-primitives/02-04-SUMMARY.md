...
provides:
  - "extensions/pi-plugins/persistence/locations.ts -- ScopedLocations brand bundle + locationsFor factory + 3 path-method helpers"
  - "extensions/pi-plugins/persistence/state-io.ts -- STATE_SCHEMA (TypeBox) + loadState + saveState + DEFAULT_STATE + STATE_VALIDATOR"
  - "extensions/pi-plugins/persistence/migrate.ts -- migrateLegacyMarketplaceRecords + persistMigratedState (IL-3 single sanctioned console.warn callsite)"
  - "extensions/pi-plugins/persistence/index.ts -- public surface barrel"
...
key-files:
  created:
    - extensions/pi-plugins/persistence/locations.ts (120 lines, 3 method-helpers + brand)
    - extensions/pi-plugins/persistence/state-io.ts (220 lines, schema + loadState + saveState)
    - extensions/pi-plugins/persistence/migrate.ts (166 lines, migration + IL-3 sanctioned warn)
  ...
  modified:
    - extensions/pi-plugins/persistence/index.ts (was placeholder, now exports public surface)

key-decisions:
  - "IL-3 sanctioned callsite location: extensions/pi-plugins/persistence/migrate.ts:persistMigratedState. The exact disable-comment incantation is `// eslint-disable-next-line no-restricted-syntax, no-console -- IL-3: load-time migrate save fail` per Phase 1 SUMMARY handoff item #2."
...
### Source code (`extensions/pi-plugins/persistence/`)
...
### Created
- `extensions/pi-plugins/persistence/locations.ts` (120 lines)
- `extensions/pi-plugins/persistence/state-io.ts` (220 lines)
- `extensions/pi-plugins/persistence/migrate.ts` (166 lines)
...
### Modified
- `extensions/pi-plugins/persistence/index.ts` -- replaced the Phase-1 placeholder `export {}` with the public-API barrel.

## Decisions Made

- **IL-3 sanctioned callsite lives at `extensions/pi-plugins/persistence/migrate.ts:persistMigratedState`** with the exact disable-comment incantation `// eslint-disable-next-line no-restricted-syntax, no-console -- IL-3: load-time migrate save fail`. Future audits can `grep -nE "no-restricted-syntax, no-console -- IL-3"` to find the single site.
...
- **Issue:** Plan acceptance criterion `grep -c "console.warn" extensions/pi-plugins/persistence/migrate.ts` should return exactly `1` (the single CALL site). The original doc-comment narrative used `console.warn` literally, returning 4. The verification block also says `grep -c "console" extensions/pi-plugins/persistence/state-io.ts` should return 0, which my initial doc-comment violated.
...
- `extensions/pi-plugins/persistence/locations.ts` exists -- FOUND
- `extensions/pi-plugins/persistence/state-io.ts` exists -- FOUND
- `extensions/pi-plugins/persistence/migrate.ts` exists -- FOUND
- `extensions/pi-plugins/persistence/index.ts` modified to re-export public surface -- VERIFIED
...
- `grep -c "console.warn" extensions/pi-plugins/persistence/migrate.ts` returns `1` -- VERIFIED (single sanctioned site)
- `grep -c "console" extensions/pi-plugins/persistence/state-io.ts` returns `0` -- VERIFIED
- `grep -c "console" extensions/pi-plugins/persistence/locations.ts` returns `0` -- VERIFIED
- IL-3 disable-comment incantation present at exact line: `extensions/pi-plugins/persistence/migrate.ts:161` -- VERIFIED
