...
- `extensions/pi-plugins/domain/resolver.ts` (553 lines) -- ResolvedPlugin union schemas + resolveStrict + resolveLoose + requireInstallable + injectable ResolveContext
- `extensions/pi-plugins/domain/index.ts` -- re-exports the resolver public API
...
- **Files modified:** `extensions/pi-plugins/domain/resolver.ts`
...
- **Files modified:** `extensions/pi-plugins/domain/resolver.ts`, `tests/domain/resolver-strict.test.ts`
...
2. **Second run (regression simulation):** Temporarily added `pluginRoot: Type.String()` to `ResolvedPluginNotInstallableSchema`. `tsc` reported:
   ...
   - `extensions/pi-plugins/domain/resolver.ts(167,3): error TS2741: Property 'pluginRoot' is missing` (bonus catch in the `notInstallable()` factory -- T-02-24 mitigation).
...
- [x] `extensions/pi-plugins/domain/resolver.ts` exists (553 lines)
...
- [x] `extensions/pi-plugins/domain/index.ts` re-exports ResolvedPlugin types + functions
