...
key-files:
  created:
    - extensions/pi-plugins/domain/name.ts
    - extensions/pi-plugins/domain/version.ts
...
- `extensions/pi-plugins/domain/name.ts` -- `assertSafeName` (RN-2) + `generatedSkillName` / `generatedCommandName` / `generatedAgentName` (RN-1).
- `extensions/pi-plugins/domain/version.ts` -- `computeHashVersion` (PI-7) + `HASH_WALK_SKIP` (D-12) + private `walkAndHash` + `normalizeBytes` (D-11).
...
- [x] `extensions/pi-plugins/domain/name.ts` exists (commit 0b68fd7)
- [x] `extensions/pi-plugins/domain/version.ts` exists (commit 45fd1d7)
...
- Plan 02-05 (resolver) can now consume `assertSafeName` from `extensions/pi-plugins/domain/name.ts` and the three RN-1 generators.
