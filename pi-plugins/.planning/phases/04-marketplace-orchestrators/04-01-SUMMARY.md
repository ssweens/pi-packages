...
requires:
  - extensions/pi-plugins/shared/errors.ts (Phase 1 existing exports)
  - extensions/pi-plugins/shared/path-safety.ts::assertPathInside (Phase 1 D-15)
  - extensions/pi-plugins/persistence/locations.ts::ScopedLocations (Phase 2)
  - extensions/pi-plugins/domain/source.ts::ParsedSource (Phase 2)
  - extensions/pi-plugins/shared/types.ts::Scope (Phase 2)
provides:
...
- **Five marketplace error classes** appended to `extensions/pi-plugins/shared/errors.ts` below the existing `errorMessage` / `appendLeakToError` / `appendLeaks` exports (no existing exports modified):
...
- `shared/errors.ts`: the three pre-existing exports (`errorMessage`, `appendLeakToError`, `appendLeaks`) are untouched; the five new classes are appended at the bottom of the file. `git diff a3a7f84^ a3a7f84 -- extensions/pi-plugins/shared/errors.ts` shows pure additions, zero deletions to pre-existing code.
- `domain/source.ts`: all eight pre-existing exports (`PathSource`, `GitHubSource`, `UnknownSource`, `ParsedSource`, `parsePluginSource`, `pathSource`, `githubSource`, plus the internal helpers) are untouched; `sourceLogical` is appended at the bottom. `git show b6975cb -- extensions/pi-plugins/domain/source.ts` shows the new function appended below `githubSource`, with one blank-line fix between switch cases (cosmetic, required by `@stylistic/padding-line-between-statements`).
...
- **Files modified:** `extensions/pi-plugins/domain/source.ts`, `tests/domain/source.test.ts`
...
Created files:
- `extensions/pi-plugins/orchestrators/types.ts` -- FOUND
- `.planning/phases/04-marketplace-orchestrators/04-01-SUMMARY.md` -- FOUND (this file)

Modified files (all exist on HEAD):
- `extensions/pi-plugins/shared/errors.ts` -- FOUND, +58 lines (commit `a3a7f84`)
- `extensions/pi-plugins/persistence/locations.ts` -- FOUND (commit `b6975cb`)
- `extensions/pi-plugins/domain/source.ts` -- FOUND (commit `b6975cb`)
