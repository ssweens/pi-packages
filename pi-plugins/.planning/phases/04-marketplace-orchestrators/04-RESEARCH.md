...
**D-09 (Staging at `<scopeRoot>/pi-plugins/sources-staging/<uuid>/`):** Same-FS guarantee by construction. New helper `sourcesStagingDir(loc, uuid)` added to `persistence/locations.ts`. UUID via `node:crypto.randomUUID()`.
...
- **Containment:** Refuse writes outside `<scopeRoot>/pi-plugins/`, `<scopeRoot>/agents/`, `<scopeRoot>/mcp.json` -- NFR-10.
...
- **Don't write to `<scopeRoot>/sources-staging/`** -- it must be `<scopeRoot>/pi-plugins/sources-staging/` (D-09). The `extensionRoot` already includes the `pi-plugins/` segment; pass through `assertPathInside(extensionRoot, candidate)`.
...
**How to avoid:** D-09 staging at `<scopeRoot>/pi-plugins/sources-staging/<uuid>/` is sibling-of-target by construction. Both dirs are under `extensionRoot`, so they share a filesystem regardless of how scope roots are mounted. Do NOT add an EXDEV fallback -- treat it as a misconfiguration error. Document the residual hazard if `extensionRoot` is itself a mountpoint on top of two filesystems (rare, but possible with bind mounts).
...
- `docs/prd/pi-plugins-prd.md` §5.1.1-§5.1.5 (MA/MR/ML/MU/MAU), §5.4 (cascade interaction), §6.2 (SC), §6.8 (RH), §6.9 (ST), §6.11 (AS), §6.12 (ES) -- authoritative spec for V1 user contract.
...
- On-disk verification (read 2026-05-10):
  - `extensions/pi-plugins/platform/git.ts` -- existing `clone`/`fetch`/`pull`/`checkout`/`resolveRef`/`listBranches`/`listRemotes` surface verified.
  - `extensions/pi-plugins/persistence/state-io.ts` -- `STATE_SCHEMA` includes `autoupdate?: boolean`, `manifestPath: string`, `marketplaceRoot: string`, `plugins: Record<...>` -- confirmed.
  - `extensions/pi-plugins/persistence/locations.ts` -- `ScopedLocations` brand + `sourceCloneDir(mp)` method exists; `sourcesDir` exposed; `agentsStagingDir` precedent for staging-dir pattern.
  - `extensions/pi-plugins/transaction/with-state-guard.ts` -- guard contract confirmed.
  - `extensions/pi-plugins/bridges/{skills,commands,agents,mcp}/unstage.ts` -- four unstage signatures verified.
  - `extensions/pi-plugins/shared/{notify,markers,errors,fs-utils,path-safety,atomic-json}.ts` -- helper surfaces all confirmed.
  - `extensions/pi-plugins/domain/{source,manifest,name,resolver}.ts` -- domain primitives confirmed.
...
- V1 reference at `git show features/initial:extensions/pi-plugins/marketplace/{add,remove,list,update,autoupdate}.ts` -- pattern reference; V1 uses `git pull --ff-only` semantics now superseded by D-14.
