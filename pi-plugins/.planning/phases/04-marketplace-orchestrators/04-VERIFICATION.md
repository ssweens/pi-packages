...
| 9   | IL-2 output channel: all messages via notify wrappers; sanctioned IL-3 console.warn isolated to legacy migration                  | VERIFIED   | `grep -rE "process\.(stdout\|stderr)\.write" extensions/pi-plugins/` returns empty; single `console.warn` site at `persistence/migrate.ts:162` matches IL-3 (legacy marketplace records migration). All orchestrators import `notifySuccess`/`notifyWarning`/`notifyError` exclusively |
