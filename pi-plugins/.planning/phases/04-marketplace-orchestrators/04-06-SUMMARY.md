...
provides:
  - removeMarketplace orchestrator (extensions/pi-plugins/orchestrators/marketplace/remove.ts)
...
- `extensions/pi-plugins/orchestrators/marketplace/remove.ts` -- `removeMarketplace` orchestrator + `RemoveMarketplaceOptions` (with `cascade` DI seam + `pi: ExtensionAPI`); cascade fan-out, post-state cleanup, MR-4 single-warning composition, RH-5 soft-dep warnings before reload-hint, NFR-5 no-network by construction.
...
- `extensions/pi-plugins/orchestrators/marketplace/index.ts` is byte-for-byte unchanged (Plan 04-10 owns the barrel)
...
- **Files modified:** `extensions/pi-plugins/orchestrators/marketplace/remove.ts`, `tests/orchestrators/marketplace/remove.test.ts`
...
- **Files modified:** `extensions/pi-plugins/orchestrators/marketplace/remove.ts`
- **Commit:** `abf912e`
...
- File exists: `extensions/pi-plugins/orchestrators/marketplace/remove.ts` -- FOUND
