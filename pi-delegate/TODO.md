...
- [x] Three surfaces, three shapes: frame = static dispatch record while running → result when done; rail = the only live view (OMP rows + current-tool hook); inspector (`ctrl+j`) = one child's transcript tail with `←→` child switching, steer/cancel/open-session. First cut rendered the same row in all three; user called it absurd and was right. No list mode — a list is the rail again.
