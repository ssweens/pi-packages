...
- **Alt+J** focuses the active list, or opens finished-child history when no children are active. It never chooses a child for you. Pi's **Ctrl+J** newline is left alone on purpose: it is the only newline that works in terminals without the kitty keyboard protocol (Shift+Enter is kitty-only), and taking it made multiline drafts impossible there.
