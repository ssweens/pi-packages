...
export class SessionManager {
  ...
  async create(params: SessionCreateParams): Promise<PiAcpSession> {
    ...
    const state = proc.getHandshakeState() as any
    if (!state || typeof state.sessionId !== 'string') {
      await proc.terminate()
      throw RequestError.internalError({}, 'Pi handshake did not provide a session id.')
    }
...
export class PiAcpSession {
  ...
  private emitFailure: unknown = null
  ...
  private emit(update: SessionUpdate): void {
    // Keep the queue moving after a failure, but retain the first failure so
    // prompt completion cannot falsely report success.
    ...
    switch (type) {
      ...
      case 'message_end': {
        ...
        if (message?.role === 'assistant' && message?.stopReason === 'error') {
          ...
          this.emit({
            sessionUpdate: 'agent_message_chunk',
            content: { type: 'text', text: `Pi provider error: ${detail}` } satisfies ContentBlock
          })
        }
        break
      }
