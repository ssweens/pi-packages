...
import { AgentDisconnectedError, AgentSpawnError, AgentStartupError, AuthPolicyError, ClaudeAcpSessionCreateTimeoutError, GeminiAcpStartupTimeoutError, PermissionDeniedError, PermissionPromptUnavailableError, UnsupportedPromptContentError, } from "../errors.js";
...
import { buildAgentSpawnOptions, readEnvCredential, resolveConfiguredAuthCredential, } from "./auth-env.js";
...
export function parseAcpJsonMessageLine(line) {
...
export class AcpClient {
    ...
    activePrompt;
    cancellingSessionIds = new Set();
    permissionAbortControllers = new Map();
    closing = false;
    agentStartedAt;
    lastAgentExit;
    lastKnownPid;
    promptPermissionFailures = new Map();
    pendingConnectionRequests = new Set();
    modelConfigIds = new Map();
    legacyModelSessionIds = new Set();
    constructor(options) {
        this.options = {
            ...options,
    ...
    updateRuntimeOptions(options) {
        const shouldRefreshPermissionPolicy = options.permissionMode !== undefined || options.nonInteractivePermissions !== undefined;
    ...
    async handleInitializeFailure(params, error) {
        ...
        if (params.launch.geminiAcp && error instanceof TimeoutError) {
            throw new GeminiAcpStartupTimeoutError(await buildGeminiAcpStartupTimeoutMessage(params.launch.spawnCommand), {
                cause: error,
                retryable: true,
            });
        }
    ...
    async setSessionModel(sessionId, modelId, controlOverride) {
        ...
        if (!control) {
            throw new RequestedModelUnsupportedError(`Cannot set model "${modelId}": the ACP session did not advertise a model config option or legacy session/set_model support.`, "missing-capability");
        }
    ...
    async setSessionModelThroughLegacyMethod(sessionId, modelId) {
        const connection = this.getConnection();
        try {
            await this.runConnectionRequest(() => connection.extMethod("session/set_model", { sessionId, modelId }));
            return undefined;
        }
        catch (error) {
    ...
    async cancel(sessionId) {
        ...
        this.abortAndDropPermissionSignal(sessionId);
        await this.runConnectionRequest(() => connection.cancel({
            sessionId,
        }));
    }
    async closeSession(sessionId) {
        const connection = this.getConnection();
        await this.runConnectionRequest(() => connection.closeSession({
            sessionId,
        }));
    ...
    async cancelActivePrompt(waitMs = 2_500) {
        const active = this.activePrompt;
        if (!active) {
            return undefined;
        }
        ...
        try {
            return await Promise.race([
                active.promise.then((response) => response, () => undefined),
                timeoutPromise,
            ]);
        }
        finally {
    ...
    async close() {
        ...
        if (this.pendingConnectionRequests.size > 0) {
            this.rejectPendingConnectionRequests(this.lastAgentExit
                ? new AgentDisconnectedError(this.lastAgentExit.reason, this.lastAgentExit.exitCode, this.lastAgentExit.signal, {
    ...
    async authenticateIfRequired(connection, methods) {
        ...
        if (!selected) {
            if (this.options.authPolicy === "fail") {
                throw new AuthPolicyError(`agent advertised auth methods [${methods.map((m) => m.id).join(", ")}] but no matching credentials found`);
            }
    ...
    recordAgentExit(reason, exitCode, signal) {
        ...
        this.rejectPendingConnectionRequests(new AgentDisconnectedError(reason, exitCode, signal, {
            outputAlreadyEmitted: Boolean(this.activePrompt),
        }));
    }
