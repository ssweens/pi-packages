...
export const REQUESTED_MODEL_UNSUPPORTED_ERROR_CODE = "ACP_MODEL_UNSUPPORTED";
...
function isRequestedModelUnsupportedReason(value) {
    return (typeof value === "string" &&
        REQUESTED_MODEL_UNSUPPORTED_REASONS.includes(value));
}
export function isRequestedModelUnsupportedError(value) {
    ...
    return (candidate.name === "RequestedModelUnsupportedError" &&
        candidate.code === REQUESTED_MODEL_UNSUPPORTED_ERROR_CODE &&
        isRequestedModelUnsupportedReason(candidate.reason));
}
export function supportsLegacyClaudeCodeModelMetadata(agentCommand) {
    if (!agentCommand) {
        return false;
    }
...
function parseAvailableModelGroup(value) {
    ...
    return models.every((model) => model !== undefined) ? models : undefined;
}
function parseAvailableModels(value) {
    ...
    if (directModels.every((model) => model !== undefined)) {
        return directModels;
    }
    const groupedModels = value.map((group) => parseAvailableModelGroup(group));
    return groupedModels.every((models) => models !== undefined)
        ? groupedModels.flat()
        : undefined;
}
...
export function modelStateFromConfigOptions(configOptions) {
...
export function modelStateFromLegacyResponse(response) {
...
export function modelStateFromSessionResponse(params) {
    return (modelStateFromConfigOptions(params.configOptions) ??
        modelStateFromLegacyResponse(params.response));
}
export function formatAvailableModelIds(models) {
    const ids = models?.availableModels
...
export function assertRequestedModelSupported(params) {
    if (!params.models) {
        ...
        throw new RequestedModelUnsupportedError(`Cannot ${action} "${params.requestedModel}": the ACP agent did not advertise model support through a session config option or legacy models metadata, and the adapter does not support a startup model flag.`, "missing-capability");
    }
    ...
    if (!advertised.has(params.requestedModel)) {
        ...
        const action = params.context === "replay" ? "replay saved model" : "apply --model";
        throw new RequestedModelUnsupportedError(`Cannot ${action} "${params.requestedModel}": the ACP agent did not advertise that model. Available models: ${formatAvailableModelIds(params.models)}.`, "unadvertised-model");
    }
    return undefined;
}
