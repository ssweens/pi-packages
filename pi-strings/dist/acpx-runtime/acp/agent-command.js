...
import { buildSpawnCommandOptions, readWindowsEnvValue, resolveWindowsExecutablePath, } from "../spawn-command-options.js";
...
export function resolveAgentCloseAfterStdinEndMs(agentCommand) {
...
export function shouldIgnoreNonJsonAgentOutputLine(agentCommand, trimmedLine) {
    const { command } = splitCommandLine(agentCommand);
    return basenameToken(command) === "qodercli" && QODER_BENIGN_STDOUT_LINES.has(trimmedLine);
}
export function isGeminiAcpCommand(command, args) {
    return (basenameToken(command) === "gemini" &&
        (args.includes("--acp") || args.includes("--experimental-acp")));
}
export function isClaudeAcpCommand(command, args) {
...
export function isCopilotAcpCommand(command, args) {
    return basenameToken(command) === "copilot" && args.includes("--acp");
}
export function isQoderAcpCommand(command, args) {
    return basenameToken(command) === "qodercli" && args.includes("--acp");
}
export function isCursorAcpCommand(command, args) {
    const commandToken = basenameToken(command);
    return commandToken === "cursor-agent" || (commandToken === "agent" && args.includes("acp"));
}
export function isDevinAcpCommand(command, args) {
    return (basenameToken(command) === "devin" &&
...
export function buildQoderAcpCommandArgs(initialArgs, options) {
    ...
    if (Array.isArray(sessionOptions?.allowedTools) &&
...
export function resolveGeminiAcpStartupTimeoutMs() {
...
export function resolveClaudeAcpSessionCreateTimeoutMs() {
...
export async function resolveGeminiCommandArgs(command, args) {
...
export async function buildGeminiAcpStartupTimeoutMessage(command) {
    const parts = [
...
export function buildClaudeAcpSessionCreateTimeoutMessage() {
    return [
...
export async function ensureCopilotAcpSupport(command) {
    const helpOutput = await readCommandOutput(command, ["--help"], COPILOT_HELP_TIMEOUT_MS);
    if (typeof helpOutput === "string" && !helpOutput.includes("--acp")) {
...
export function buildClaudeCodeOptionsMeta(options, isolateUserSettings = false) {
...
export function resolveClaudeCodeSettingSources(env = process.env) {
...
export function resolveClaudeCodeExecutable(platform = process.platform, env = process.env) {
    if (platform !== "win32") {
        return undefined;
    }
