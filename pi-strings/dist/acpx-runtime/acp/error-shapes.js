...
export function toAcpErrorPayload(value) {
...
function extractNestedAcpError(record, depth) {
    for (const key of ["error", "acp", "cause"]) {
        if (key in record) {
...
function isSessionNotFoundText(value) {
    ...
    return (normalized.includes("resource_not_found") ||
...
export function extractAcpError(error) {
    return extractAcpErrorInternal(error, 0);
}
export function isAcpResourceNotFoundError(error) {
